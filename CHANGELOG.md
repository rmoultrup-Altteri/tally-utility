...
**What it builds.** 16 TAC §7.45 limits how far back a utility may *bill* a customer it under-charged, and separately how far back it may *enforce collection* — the two diverge, and for a non-registering meter three months may be billed and none of it ever enforced. `backbilling_cap_rules` carries both bounds with explicit non-null scope enums on each side (R-20 ruled this for the enforceable side against the NULL-poisoning class; F-7 found the billable side had the same three-shape problem and no discriminator, where a NULL read as "no limit" bills past statutory authority). Texas gas seeds for *both* customer classes — the unprotected rows written explicitly `uncapped` rather than left absent, so a missing rule stays an error rather than a silent permission. `backbill_cause` + `anchor_date` on the correction target, findings-based per R-19, with the under-reach override in **database-stamped write-once columns** rather than `invoice_events`, which `tally_app` can write itself. Two append-only tables: `correction_run_target_events` for the setup-time decisions that predate any correction invoice, and `backbilling_period_evaluations` for R-25's per-period evidence. The R-30 read classification as a platform-fixed function, not a table — a tenant able to reclassify `ami_offline` as beyond its control would be granting itself a disconnection right the rule does not give. `tenants.regulatory_class_mode` ships CCK-14's simple default with the volumetric mode declared but **refusing** rather than silently degrading. The v5.4.2-10 target freeze extended to the two new columns (F-4, an amendment to R-19 stated in the header). The AC-32 tail, shown to RAISE on four planted drifts.

**Round 1 found four critical/high defects. All are fixed; two were structural.**

**The trim was recorded and never applied.** The evaluator computed the forfeited amount, wrote it to the permanent evidence row, raised a NOTICE announcing it — and the invoice then issued at the *full* delta, because the gate only refused when the trim consumed everything. By this patch's own header standard ("a guard that fails open is worse than none because it is trusted") that is the worst available outcome: the log said forfeited, the bill was not. It also had no fixed point — the trim is proportional to a delta derived from the draft, so re-drafting at the trimmed figure re-prorates the already-trimmed number ($31 → $9 forfeited; redraft at $22 → $6.39). Now refused outright pending Q-C.

**`amount_due` is caller-set and tied to nothing — found independently by both reviewers**, which by `reviewer-fixes-need-measuring` makes it near-certain. Nothing in 21,916 lines derives it from `invoice_line_items`. Every arithmetic test in the gate read it on both sides of the comparison. **The first fix was the same defect mirrored:** told amount_due could not be trusted, the revision read only the line items — but `void_invoice()` reverses exactly `-(amount_due)`, so that column reaches the ledger whatever the lines say, and a correction with honest $105 of lines and `amount_due` at $5,000 issued clean. Reproduced by the author before it shipped. The measure is now the **greater of the two paths**. Requiring the two to be *equal* was tried and **rejected on measurement**: batteries 10 and 11 build invoices with `amount_due` 0 against non-zero lines, and equality refused them — confirmed as the cause rather than test contamination before the reasoning was written into the patch.

**The gate asked what the bill was called, not what it did.** Keyed on `invoice_type = 'correction'`, it was walked past three ways: a `duplicate` and a `credit_memo` may both legally carry `replaces_invoice_id` (and v5.4.2-10 *explicitly exempts both* from its coordinate binding), and — needing no `replaces_invoice_id` at all — voiding a bill and issuing a fresh `regular` invoice for the same premise and period. The patch had quoted R-22's "the regulated act is the charge, not the void" to justify keeping the gate out of `void_invoice()`, then keyed the gate on a type label one function later. Re-keyed on the facts: was this premise and period billed before, and does this bill charge more. A rebill that charges *less* still passes.

**The target's identity was unbound to the bill it corrects.** `meter_id` and `customer_id` are plain FKs and both choose the cap — a sibling meter at the same address with an older test date lengthens the window, and under `explicit_class` naming a large-commercial customer on a residential customer's bill resolves `unprotected`, which is billable-uncapped. Both now bound to the voided bill. The evidence row also records the meter and the `last_test_date` that produced its window, because that field is mutable and the window was otherwise not re-derivable — which is the entire purpose of the record.

**Two questions now block, recorded in GBM `2da510b`** (`kyle-questions-2026-09-22-a2-straddle-and-last-test.md`). **Q-C, the straddling period:** R-25 makes the billing period the unit and R-23 forfeits the remainder; between them they settle only a period wholly inside the window and one wholly outside it. With monthly periods and a mid-month window the straddle is the common case, not an edge case. Three candidate rules recorded; engineering preference (forfeit whole) offered as input, not a position. **Q-D, which test date governs:** implemented against `meters.last_test_date`, the ordinary sequence — test the meter, record the result, raise the correction — *cannot complete*, because recording the discovery test opens the window on the anchor and every period precedes it. A `meter_error` correction is possible only if the test justifying it is never recorded. The reading that resolves it is "the last test at which the meter was found **accurate**". **If that is right, CI-091's append-only test history becomes a prerequisite for A-2 rather than the patch behind it** — `last_test_date` is a single field each test overwrites, so the prior test is not recoverable from the schema. R-31's bar on deriving a test date from `test_interval_months` is honoured either way: where no prior test is recorded, the patch refuses rather than inferring.

**Recorded for the register, pre-existing and not A-2's:** `tally_app` can post a charge directly into `account_ledger` outside any invoice gate, so the ledger and the bill are both application-asserted; and `invoices.amount_due` has no tie to its line items, which A-2 is the first thing to rest a statutory gate on (residual R9).

**Verified:** strict apply ×2 (`search_path=''`, `check_function_bodies=on`) clean and idempotent; **battery-12 66 PASS** (group J pins every round-1 finding so none can regress); regressions **28 / 58 / 41**, each at its recorded count; every round-1 reviewer repro now refused. Catalog +3 tables, +3 policies, +3 FORCE RLS, +6 triggers, +12 functions, +8 FKs.

**Not done:** round 2 reviews; the mirror into `tu.sql`; DEPLOY-VERIFICATION; the CI re-grades (CI-008, CI-092, CI-091); the Appendix and DECISION-LOG entries. All of it waits on Kyle, because Q-C decides whether the trim mechanism exists and Q-D may reorder the next two patches.

---

## 2026-09-22 — Kyle's A-2 fence lifted; the patch consolidated into a spec; and the tenant-blind foreign key turns out to be 229 of them

**What was done:** No DDL. A documentation and scoping session across both repos, in three parts.

**AC-32 socialized (the session's original task).** The contract requiring every future patch to call `assert_tenant_isolation_invariants()` in its tail was recorded three times after v5.4.2-11 — Appendix A-23 item 1g, the parity plan's Wave 3 landing line, ingestion Section BB — but all three are *historical* entries about what -11 did, and none sat where the next patch author reads their obligations. The parity plan's per-wave acceptance sentence now carries it in the imperative, with its reason (invariants hold at apply time and drift at the next `CREATE`) and the two corollaries the -11 rounds paid for: write the check against the property it certifies rather than a narrower key that selects today's rows, and prove it RAISEs on planted drift. A fifth standing risk records the shape — isolation decays by default, not by mistake, and both leaks -11 closed were found by review rather than by the appendix that had looked straight at that surface. Put on the acceptance line rather than per A-item because acceptance is inherited by items added or re-sequenced later. GBM `e25fa95`, ingestion Section BC.

**Kyle closed Part 4 on 2026-09-14 — discovered mid-session when a push was rejected.** R-26…R-31 land the last seven items and lift the fence: A-2 is ready to draft. Jurisdiction fallback goes on the cap table itself (`jurisdiction_id` NULL = the state default, `jurisdictions` untouched, A-8 decoupled); the under-reach warning ships inside A-2 with its operator surface in first release; the (4)(E)(vi) clause and the (v)(I) anchor go to tariff counsel with nothing built on them; the (4)(E)(vii) determination is classified from `meter_readings.access_status` / `estimation_reason` through a platform-fixed three-bucket mapping; CI-091 becomes an append-only meter test history table, its own patch immediately behind A-2, required before the first gas tenant goes live. Five refinements carry an engineering reading rather than Kyle's ruling and are marked as such.

**The A-2 implementation brief** (`gas-billing-memory/application/a2-implementation-brief-2026-09-21.md`). Kyle's record is complete but is not a spec: R-19…R-31 with three amendments, the class dimension in a second record, and Parts 1–3 cited against the old snapshot. Consolidated and re-based against `tu.sql` — `meter_readings` CHECKs at 3575/3578 carry exactly the values R-30's buckets map; `meters` test columns at 3691–3694 hold; `rate_items.applies_to_customer_types` is dropped at 15523 and the defect lives on `rate_item_versions` at 15376. **Eight collisions between the rulings and the shipped schema**, four needing a decision: gate (ii) events have no invoice to attach to (`invoice_events.invoice_id` is NOT NULL and `correction_invoice_id` is NULL until Phase 7); the event log is app-insertable, so the under-reach override is assertable by `tally_app` and is not a guard's fact; `backbill_cause` falls outside the column-list AC-31 target freeze **and** R-19's freeze point (post) is later than the snapshot's, which would let frozen evidence describe a window the target no longer claims; and the CCK substrate does not exist, so the cap key's class dimension needs a v1 scope.

**Ryan's calls (2026-09-22):** ship CCK-14's `all_non_residential_protected` default rather than building the resolver (halves the patch; errs toward protection, which is never a violation); give the under-reach override a database-stamped write-once home; give the correction-run target rows their own append-only log; freeze the cause at the snapshot and restrict the override to evidentiary-impossibility reason codes — both narrower than the standing rulings, both put to Kyle in `kyle-questions-2026-09-22-a2-override-and-cause-freeze.md`, both built narrow meanwhile because widening a CHECK is one line while narrowing means re-judging rows already written.

**The finding that outgrew its question.** A single adjacent defect — a service address able to name another utility's city — was checked and turned out to be schema-wide. **Verified against a fresh container build of the committed tu.sql (zero errors in `docker logs`) by querying `pg_constraint`, not by reading the file: 79 tenant-scoped tables, 254 foreign keys between two of them, 25 tenant-safe, 229 blind, across 67 tables, with only 15 targets carrying the `UNIQUE (id, tenant_id)` a composite FK must reference.** A static pass the day before had said 237; it double-counted constraints dropped and re-added by later patches, and 229 is the figure to work from. Exposure is a class of accident rather than an open door — RLS still governs reads, so the routes are imports and backfills running with elevated rights, platform admins, and application bugs — but the schema is the only enforcement layer and RLS says nothing about whether a link points somewhere coherent. Tiered by consequence in `tenant-blind-foreign-keys-2026-09-22.md`, with the full 229-row inventory; 96 of them (42%) are `created_by` / `approved_by` audit columns pointing at `users`.

**And the modelling question underneath it.** Ryan: a city does not change because two utilities serve it. `jurisdictions` is fully tenant-scoped and mixes place identity with per-utility configuration, so two utilities in Dallas hold two unrelated Dallas rows with no canonical form. The shared-place split is recommended — and is not a foreign pattern, since `program_types` is already shared reference data — but routed to A-8 rather than A-2, with the wrinkle recorded that a jurisdiction is not always a municipality. `jurisdictions-shared-place-modelling-2026-09-22.md`.

**Verified:** container rebuilt from the committed `tu.sql` (21,916 lines), `PostgreSQL init process complete`, zero error/fatal lines, catalog queried directly for every figure above.

**Not done:** no DDL drafted. A-2 is ready to draft behind one open scope call (where the FK remediation patch sits relative to A-2). AC-29 / AC-30 / AC-31 remain un-socialized — they bind individual write paths rather than the shape of every patch, so their likely home is the register and the fixture catalog, not the acceptance line.

---

## 2026-09-18 (UI concepts) — a runnable first-iteration UI over the spec, and the fixture defects building it exposed

**What was done:** A separate workstream from the schema series — `ui-concepts/`, a nested Next.js 16 / React 19 / Tailwind v4 / Zod project on fixtures, no database. Fifteen screens covering all ten from the domain report's Screen list, built to see the product working and react to it rather than to ship. Fixtures transcribed from the real CHECK vocabularies in `sql/tu.sql` (186 enum sets, extracted rather than trusted from `schema.sql`, which carries none) and from the locked T8 rulings. Runs on port 4182; `pnpm dev`, `pnpm build`, `pnpm check:fixtures`.

**The screens that carry the domain rather than decorate it.** `/invoices/[id]` is the tariff inspector — Ccf through multiplier, BTU factor, the rider DAG in topological order, with the derivation rail beside the printed bill. `/invoices/[id]/diff` shows both snapshotted input sets and the rate-date election as a recorded operator choice. `/rates/pga` is a bi-temporal lattice: service months across, recorded instants down, each cell the factor a run would have used pricing that month at that instant. Filings normally land days ahead of the month they govern, so the grid is a staircase; January's, recorded 46 days late, lands two rows below it, and one cell above it is marked — the January run priced the whole cycle out of a coordinate where no version stood, carrying December forward instead of refusing. `/rates/sandbox` moves a rate and rehearses it against a closed cycle's actual volumes, reporting the distribution rather than an average: the opening rate-design shift crosses over at 166 therms, lifts 2,474 bills and lowers 701, where the average is sixty cents and hides all of it. `/collections` orders by what an operator may lawfully do today rather than by who owes most, because 16 TAC §7.460 is forecast-driven and covers residential service only; the bankruptcy stay is lifted off the worklist entirely rather than shown as a flag, since a flag is something an operator can talk themselves past.

**Theme.** One Tailwind v4 `@theme` block, tokens named for their role in gas billing rather than by hue. Every colour is declared once as `light-dark(day, night)`, so the dark theme is the second half of each token rather than a second palette to keep in sync. Three appearance states (System is the absence of `data-theme` and needs no JavaScript); a bootstrap expression applies a stored pin before first paint. **The bill is exempt** — `stock` pins `color-scheme: light` inside the sheet, so a statement stays white paper with dark ink in both themes without any of its markup knowing themes exist.

**Five fixture defects found, four of them by writing checks rather than by looking.** (1) The signature bill INV-2026-02-004182 did not foot: lines summed to $158.34 while the header claimed $218.94 and the franchise line agreed with neither. (2) INV-2026-02-062118 had a $1,164.02 header and no line items at all, because G-1 had never existed — the one commercial account on the site was billed by a schedule with no rates in it. (3) The bill's derivation rail applied the meter multiplier **twice**, and looked correct on every account for as long as every multiplier was 1.0000; a 1.5-inch meter at 2.0000 made it print an equation that does not produce its own answer. (4) The dashboard rate card claimed an as-billed WNA of −0.014200 that appears on no invoice line — precisely the T8-8 failure the card exists to prevent. (5) `BR-2026-01-04` was cited by three screens and did not exist; adding it exposed a sixth, a narrative contradiction no schema constraint would catch (the PGA console said 412 bills where the run that priced them billed 3,412).

**`pnpm check:fixtures`** now asserts the identities the screens display but nothing enforces — referential integrity, the gas derivation chain, invoice and line arithmetic, tier quantities against the commodity line, rate version chains and bi-temporal non-ambiguity, lifecycle rules including one-live-rebill-per-lineage, favorites targets, and the cross-fixture numbers one screen states in prose about another's rows. **Mutation-tested, not assumed:** breaking a cent, a multiplier, a day count, a version chain, an inverted filter predicate or a narrative total each produce a finding. The first run of that test found two checks that did not fire, one a ±0.011 tolerance letting a one-cent `taxable_amount` drift through — a sum of cent-rounded figures is an exact identity, and a tolerance there hides the only thing it was watching for. No new dependency: fifteen lines of resolver hook teach Node the `@/` alias.

**Locked rulings implemented:** T8-1 (quick-find on every page, five match fields, no advanced-search screen), T8-2 (favorites, below), T8-4 (portlets placed by item type, never by row severity), T8-5 (revenue pair, voids net at void date, **no ratio** between the figures), T8-6/7 (AR aged by invoice, drilldowns must sum to the tile), T8-8 (rate card reads as-billed from `invoice_line_items.rate`, drift marker only where it differs). **T8-2's record-level exclusion is enforced by shape:** `lib/views.ts` defines a three-variant favorite target union with no `entity_type` / `entity_id` pair anywhere, because a polymorphic favorite reopens record-level the first time somebody writes `entity_type = 'customer'` and no review catches that reliably. A saved search and a saved filtered view are one object; queue filter state lives in the URL, so a saved view is a shareable link with a name on it.

**Accessibility.** All rendered text meets WCAG AA in both themes, measured by pixel conversion rather than asserted — the first attempt parsed `oklch()` as RGB and scored uniform nonsense. Clearing it moved three things, two latent in the light theme since the first commit: accent split into a fill role and a text role (a blue dark enough to carry white button text is unreadable as a link on a dark panel); voided records now carry their meaning in the strikethrough rather than in being faint; `--color-ink-muted` is reserved for placeholders and `aria-hidden` separators, so the em-dash meaning "no value recorded" sits at `ink-tertiary` or above.

**Headline failed approaches:** CSS Modules where the instruction was a Tailwind *theme* (utility classes must not scatter — long strings live inside primitives, screens compose `<Money>` / `<StateFlag>` / `<Rail>`); reading the rate version chain at the viewing coordinate instead of the instant the correction run actually executes; labelling a gas usage chart by month name when bill periods run 28–33 days and HDD must be computed per read window; a contrast audit that parses `oklch()` as RGB; a mutation harness whose patterns end in a quote; a `light-dark()` value inside an element that pins `color-scheme` (it can only ever resolve one way); and trusting HTTP 200 — every defect above was caught by looking at a rendered page or by a check, never by a status code.

**Residuals / not built:** interactions are presentational (nothing writes); AR bucket drilldown lists; the keyboard affordances are drawn (`j`/`k`, `⌘K`) but inert; Screen 6's standalone portal view, hover tooltip, click-a-bar explainer and balance-point personalization line; Screen 4's rate item and tier editors, version diff and rollback (the sandbox half is built, the editor half is not); the shell overflows horizontally below ~600px on every screen, which is a decision to make rather than an accident; the G-1 tariff rates, the Texas collections parameters and the January exposure volumes are prototype inventions, not canonical facts, and should not be ingested as such.

**Next:** decide whether this iteration is the direction before building further on it. If yes, the ordered queue is the Screen 4 editor half, Screen 6's portal view, AR drilldowns, then wiring keyboard affordances.

## 2026-09-09 (definer hygiene) — v5.4.2-11: the surfaces that ran with `tally`'s rights, and a re-runnable tenant-isolation gate

**What was done:** `tally` is a superuser with `rolbypassrls`, so anything executing with its rights reads every tenant. `sql/v5.4.2-11-definer-hygiene-and-view-rls.sql` (1,100 lines, md5 `a275c67d…`) closes every such surface found. **Ten items.** The two owner-rights views (`adhoc_void_pending_review`, `void_released_read_alerts`) get `security_invoker` — measured before the fix, tenant 1 read tenant 2's parked charge by name. `get_user_tenant_id` / `is_platform_admin` stay SECURITY DEFINER (the `users` policy calls them, so invoker rights would recurse) but get `public.users` qualified and `SET search_path = ''`; that lifts A-23 (1c), since while they were unpinned NO session pinned to `''` could read ANY RLS table. `validate_custom_fields` → invoker plus an explicit tenant refusal (invoker rights alone returned `[]` — a false VALID — and as a definer it returned the other tenant's field keys and labels). `get_correction_rate_date` → invoker (as a definer it resolved any tenant's rate-date election from a bare id pair; its only in-schema caller proves visibility first, so nothing is lost). EXECUTE revoked from PUBLIC on all five definer-family functions **and** from the global default that would hand it to the next one — the schema-qualified form of that revoke is a silent no-op and the header carries the measurement. `anomalies.entity_type` gets a fifteen-value singular domain derived from the 37 `anomaly_type` values. **Round 1 found a CRITICAL the patch could not see:** four statistics matviews (`compliance_`, `credit_aging_`, `payment_health_`, `usage_statistics`), owner-refreshed under BYPASSRLS and grantable but never RLS-able, handed every tenant's tax-exemption, NSF, credit and consumption data to `tally_app` — revoked outright (Ryan; wrapping was declined, and a `security_invoker` wrapper is impossible). **The verification became a function:** `assert_tenant_isolation_invariants()`, owner-only and invoker-rights, which every future patch calls in its tail and the container build runs last — because the invariants are true at apply time and drift at the next `CREATE`. Round 3 folded TABLES into it (RLS enabled + forced + at least one permissive policy + every permissive policy canonical in both `USING` and `WITH CHECK`, partitions included). Mirrored (tu.sql 21,165 → **21,916**); fresh build verified by hash and catalog (+1 function exactly); battery **41** green on clean and on the build (`tests/v5.4.2-11/`), with an 11-check pre-patch probe that fails on the -10 build in both directions.

**Reviews:** five hash-frozen rounds, both reviewers (`133546f8` → `5b27cc38` → `1a0f6c84` → `c35698ac` → `a275c67d`). Round 1: the matview CRITICAL (Codex) / HIGH (Fable), found independently, plus Fable MEDIUM on the default privilege that reopens PUBLIC EXECUTE for the next definer. Round 2: both "sound enough to mirror"; Fable found the `has_table_privilege` blindness to column grants, Codex found the per-schema default ACL shadowing the global fix. Round 3: **both independently found that the assertion never checked TABLES** (Codex HIGH with a full cross-tenant data repro). Round 4: **both independently found the `coalesce(polqual, polwithcheck)` write-side hole** — a canonical `USING` with `WITH CHECK (true)` is read-isolated and write-open; Codex added two false positives (RESTRICTIVE policies wrongly refused; operand order). Round 5: both "confirmed, no findings". Three independent convergences; rounds 4 and 5 each found a defect in code written *during* the review.

**Reviewer findings corrected before folding** (the code would have been wrong): Fable's `ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE ...` is a silent no-op (PUBLIC's EXECUTE lives in the global, schema-less default; only the unqualified `FOR ROLE` form works — both measured); Fable's "all 81 policies render to one canonical string" is two, since `tenants` is keyed on `id`; Codex's per-schema-over-global "precedence" is actually union; both reviewers' proposed `security_invoker` matview wrapper is impossible.

**Decisions:** D-2026-09-09-01…-09; **AC-32** (every patch calls the assertion; the table of what the database now refuses to certify). Ryan's calls: invoker over revoke on the resolver; the PUBLIC cleanup in scope; fifteen values, no `adhoc_charge`/`service_order`; revoke over wrap on the matviews; the assert function over an event trigger; fold the table checks rather than defer to a `-12`; record the broken `refresh_statistics_views()` rather than fix it. **Headline failed approaches:** a self-check filtered `relkind = 'v'` while certifying "views run with invoker rights" — it could not see the four matviews one relkind away, and the same narrowing hid the tables two relkinds away; `has_table_privilege` is blind to column-level grants; `coalesce(polqual, polwithcheck)` never inspects `WITH CHECK` while `USING` is non-NULL; a `RETURNING` on a cross-tenant INSERT masks a write hole behind what looks like a blocked write; asserting a performance claim ("evaluated once per query") that measurement contradicted (+45% on a seq-scan filter).

**Residuals:** R1 `void_invoice` keeps `public, pg_temp` (body fully qualified — the TEMP premise does not hold on clones: `CREATE DATABASE ... TEMPLATE` does not copy `datacl`, so every battery in this project has run with TEMP held, unlike the deployed database). R2 the ~190 trigger functions are not re-pinned to `''` though A-23 (1c) now permits it. R3 non-definer functions unaudited for PUBLIC EXECUTE. R4 `anomalies.entity_id` has no polymorphic FK. R5 the matviews have no tenant-scoped read path, and `refresh_statistics_views()` has never worked for anyone (`ON CONFLICT (view_name)` ambiguous with the `RETURNS TABLE` OUT variable) — the dashboard work fixes both. R6 a permissive policy with neither `USING` nor `WITH CHECK` passes the assertion (verified fail-closed). R7 the matview and definer checks name `tally_app`/PUBLIC, so a future second application role would be unwatched. Design limit: the key is the column name `tenant_id`.

**Next:** socialize AC-29–AC-32 into the parity plan and ingestion; consolidate the Kyle brief candidates; A-10 when the finance-gate question is settled; A-2 when Part 4 is empty; A-8 when its brief is answered.

## 2026-09-08 (A-3 follow-up) — v5.4.2-10: the snapshot's coordinate pair is bound to facts the database holds

**What was done:** Kyle-independent work while A-2 waits on his Part 4. Ryan ruled strict on both drafting calls (an ordinary bill is priced at exactly `period_end`; a correction with no correction-run target row is refused). `sql/v5.4.2-10-snapshot-coordinate-binding.sql` (980 lines, md5 `f143c1cb…`) closes A-23 (1e): `invoice_calculation_snapshots.(valid_at, recorded_at)` is now checked at insert and issuance — ordinary: `valid_at = period_end`, `recorded_at` within `[run.started_at | invoice.created_at, now()]`; correction: on a correction run with a target row, replaced bill visible + same-tenant + void + once issued, `valid_at = get_correction_rate_date()`, `recorded_at` in the run's window or exactly the original's coordinate, one live rebill per lineage. The inputs the check reads are the database's and frozen: `started_at` / `created_at` / new `first_issued_at` stamped and write-once; run election + `run_type`, target election, and a snapshotted draft's type/lineage/period frozen; `replaces_invoice_id` FK made tenant-composite. Concurrency: run row FOR KEY SHARE (heartbeats pass), row-version mutexes on invoice / lineage root / replaced bill / target, isolation pin only on the run guard, canonical lock order in AC-31. Mirrored (tu.sql 20,407 → 21,165); fresh build verified (catalog +6 functions / +4 triggers); battery 58 green on clean, pre-seeded and the build (`tests/v5.4.2-10/`); nine live two-session shapes green.

**Reviews:** five hash-frozen rounds, both reviewers (`bb7cdbd9` → `9160f8af` → `2530daf7` → `2d0ece3c` → `f143c1cb`). Round 1: Fable CRITICAL (`created_at` movable on an unsnapshotted draft), HIGH ×2 (a live draft as the "original"; a deletable draft snapshot as replay anchor); Codex HIGH (the isolation pin refused RR edits of never-snapshotted rows); plus the author's heartbeat-blocking probe. Round 2: a held-then-voided never-billed discard as the original; per-run uniqueness. Round 3: both found the per-hop lineage gap independently. Round 4: untyped rebills; the tenant-blind FK. Round 5: both "sound enough to mirror", no findings.
...
## 2026-09-08 (housekeeping while A-2 waits on Kyle) — batteries rescued into `tests/`, HANDOFF re-based on A-9, container verified

**What was done:** No schema change. Three hygiene items, done before any Wave 3 work: **(1)** `tests/` created — the -09 battery (28 checks), its review brief, reviewer seed, precondition seed, nine repros and four author attacks rescued from the -09 session's scratchpad into `tests/v5.4.2-09/`; the battery re-run from its new home against a clone of the `tally-pg` build: **28 PASS, ROLLBACK**. The -07 and -08 batteries (161 / 173 checks) were already gone with their scratchpads — recorded as lost; from -09 on batteries are committed. The three frozen -09 review copies were not rescued: round 3's hash (`90af192e…`) is the committed patch. `tests/README.md` records the run recipe and the battery conventions. **(2)** This CHANGELOG repaired: the -09 session's entry had been written INTO the -08 entry (the -08 header was replaced, the -08 body left under the -09 header, and the -08 "Next: unchanged" line stood as the -09 one). The -08 header is restored above its own body and -09 has its own **Next**. No text was removed. **(3)** `HANDOFF.md` rewritten — it still described A-7 as the latest landing. **Container verified**, not assumed: `/docker-entrypoint-initdb.d/01_schema.sql` in `tally-pg` hashes identical to the committed `sql/tu.sql` (`cacefef2…`), and the live catalog matches DEPLOY-VERIFICATION exactly (82 tables / 81 policies / 80 FORCE RLS / 357 CHECKs / 357 FKs / 10 EXCLUDE / 59 UNIQUEs / 251 triggers, 183 ENABLE ALWAYS / 571 indexes / 386 functions; TEMP revoked; both A-9 accessors and the renewal view present).

**Decisions:** none new. **Failed approaches:** a disk-wide `find` for stray batteries hung the shell — search the known scratchpad roots only.

**Next:** Kyle-independent queue, in order: the A-3 `valid_at` binding (A-23 1e) with the small structural residuals folded in; socialize the -08/-09 contracts (AC-29 / AC-30) into the parity plan and ingestion; consolidate the Kyle brief candidates. A-10 when the finance-gate question is settled; A-2 when Part 4 is empty.

---

## 2026-09-04 (A-9) — v5.4.2-09: tax exemption certificate evidence and renewal surfacing

**What was done:** Kyle's answers came back; A-2 stays fenced by his own record's Part 4 stop (five items open), so the session did the Ryan-assigned half of item 2 (jurisdictions granularity verified against tu.sql, note pushed to GBM `259efe2` — the cap table needs fallback semantics or A-8, the `applies_to_customer_types` defect re-targeted to `franchise_fee_rules` + `rate_item_versions`, the AR-tile hazard confirmed) and then landed Wave 3's A-9. A-1 had already rebuilt `customer_tax_exemptions`, so the patch is CI-046's remainder only: the per-category certificate-required knob (settings accessor, default TRUE per R-13/Rule 3.287, wrong shape at either path level raises), the certificate-evidence gate folded into the lifecycle trigger (entry to any asserted state needs `certificate_number`/`certificate_url` with ≥1 alphanumeric character; drafts free; asserted rows never re-judged; flips audited by `tenant_configuration_history`), the self-verifying precondition blocking on every evidence-less CURRENT head (active/expired/revoked — as_of doesn't filter status), and the renewal queue (`tax_exemption_renewal_notice_days` default 60 + `customer_tax_exemptions_renewal_due`: enters at exactly N days remaining, lapsed rows stay until their expiry succession). Three hash-frozen review rounds (`b8318398` → `af834c44` → `90af192e`; Fable + Codex): round 1 caught a CRITICAL (btrim passed a tab as evidence) and a HIGH (precondition ignored current revoked heads); round 2 caught the author bending a display column to a wrong `<` boundary instead of fixing it, and the ancestor-shape hole. Battery 28 green (clean + pre-seeded); mirrored (tu.sql 20,407); fresh-build verified (catalog parity +2 functions; battery green on the build).

**Decisions:** D-2026-09-04-01…-07; AC-30. NOT landed by decision: stored renewal fields (the `remitted_on` lesson), a legacy `is_tax_exempt` guard (display-only; residual under CI-046), per-category validity seeds (the R-21 lesson). Headline failed approaches: btrim as a blank-check (whitespace family — whitelist one alnum instead of blacklisting invisibles); fixing an off-by-one's display instead of its boundary (a battery check that asserts a column against its own formula proves nothing).

**Next:** Wave 3 has one unfenced item left — A-10 (revenue distribution matrix; composes with the GL neighbours behind the finance gate, so it may slip). A-2 waits on Kyle's Part 4 items 3–7 and the item-2 semantics pick (GBM `application/kyle-outstanding-2026-09-05.md` indexes everything on his desk); A-8 held for its brief. Kyle-independent work queued: the A-3 `valid_at` binding (A-23 1e), the small structural residuals (four pre-existing views' `security_invoker`, A-23's three unpinned definers, `anomalies.entity_type` CHECK), a `tests/` home for the batteries, and this HANDOFF — which was NOT rewritten in this session.
