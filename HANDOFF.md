# Handoff: A-4 LANDED (v5.4.2-01) — next is Phase 4 Wave 1, A-1

**Generated**: 2026-08-20 (end of A-4)
**Branch**: tally-utility `main` (pushed) · gas-billing-memory `ryan` (pushed; 16 commits ahead of `origin/main`)
...
- [ ] Flagged from A-4 (later factual-defect set): CHECK `status='void' ⇔ voided_at IS NOT NULL`; forward ordering among issued statuses; `rate_schedules` duplicate policy columns (from -02).
...
**Working**: `tu.sql` at v5.2.1 + v5.4.0-00→-06 + v5.4.1-01/-02 + v5.4.2-01, 14,030 lines; 66 tables / 229 CHECKs / 147 triggers / 275 FKs; container `tally-pg` is a fresh build of the committed tu.sql.
...
-- Carve-out: current_setting('app.void_operation', true) = 'true'  -- set+cleared by void_invoice(); caller-settable (AC-12)
...
2. Same loop: live-test → two independent reviews (fresh-load scratch DBs) → fresh rebuild → mirror → DEPLOY-VERIFICATION → GBM re-grades + Section AO + CHANGELOGs → commit + push both.
3. Add caller obligations to APPLICATION-CONTRACTS (AC-14+); decisions to DECISION-LOG.

## Warnings

- tu.sql APPEND-ONLY; anchors 337/3600/3679. Everything under `search_path = ''`.
