# Handoff: v5.4.2-10 landed (A-3 follow-up — the snapshot coordinate pair is bound; A-23 1e closed); definer-hygiene patch (v5.4.2-11) read but NOT drafted; A-2 fenced on Kyle's Part 4

**Generated**: 2026-09-08 (wrap — v5.4.2-10 landed after five hash-frozen review rounds; housekeeping earlier the same day; the v5.4.2-11 reading pass was done and interrupted before any file was written)
...
**2026-09-08 (late, v5.4.2-11 reading pass — nothing written):**
- [x] Inventory on the -10 build: five SECURITY DEFINER functions remain — `get_user_tenant_id` (SQL, unpinned, `FROM users` unqualified), `is_platform_admin` (same), `validate_custom_fields` (plpgsql, unpinned, `FROM custom_field_definitions` unqualified; no caller in tu.sql — app-facing), `get_correction_rate_date` (pinned `public, pg_temp`, unqualified body, definer — reads any tenant's target by id), `void_invoice` (pinned, qualified — leave). Full definitions saved in the session scratchpad `defs-11.sql` (re-pull with `pg_get_functiondef` if gone).
- [x] Views: `adhoc_void_pending_review` and `void_released_read_alerts` have NO `security_invoker` and are owned by the superuser — **they bypass RLS; `tally_app` can read every tenant's rows through them** (a real leak, not hygiene). The other three views already have it.
- [x] `anomalies.entity_type`: NOT NULL text, no CHECK, no COMMENT, no domain anywhere in tu.sql or GBM; the only writer is `enforce_service_type_change` → `'rate_schedule'`. A CHECK is a drafting call (candidates: customer, service_location, meter, meter_reading, invoice, billing_run, rate_schedule, adhoc_charge, payment, deposit) — say so in the header; self-verifying refusal if existing rows carry other values.
...
1. **Draft `v5.4.2-11` (definer hygiene + the two leaking views)** from the reading pass above — plan: (a) `ALTER VIEW … SET (security_invoker = true)` on `adhoc_void_pending_review` and `void_released_read_alerts` (the leak; test cross-tenant SELECT as `tally_app` before/after); (b) re-issue `get_user_tenant_id` / `is_platform_admin` as SECURITY DEFINER (they MUST stay definer — the `users` policy calls them; definer SQL functions are never inlined, so the pin costs nothing) with `public.users` qualified and `SET search_path = ''`; (c) `validate_custom_fields` → SECURITY INVOKER, `public.custom_field_definitions` qualified, pinned, plus a refusal when `p_tenant_id` is not the caller's tenant and the caller is not platform admin (invoker rights alone would return "valid" for a foreign tenant id); (d) `get_correction_rate_date` → SECURITY INVOKER, body qualified, pin kept (its only caller is -10's helper, after the visibility check; a foreign id then returns NULL); (e) `anomalies.entity_type` CHECK with the domain stated as a drafting call; (f) A-23 (1c) rule lifts: after this patch a trigger function may pin `''` — record it, do NOT re-pin the existing ones here. Verify: strict apply ×2; both existing batteries (-09: 28, -10: 58) green as `tally_app` (they exercise RLS through the helpers); a `SET search_path = ''` session as `tally_app` selecting an RLS table; the view leak closed. Then the usual loop (brief → freeze → two reviewers → mirror → build → docs → `tests/v5.4.2-11/`).
