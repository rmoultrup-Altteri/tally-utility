...
**Method.** Fresh clone (TEMPLATE tally) → strict apply ×2 (`search_path=''`, `check_function_bodies=on`), clean and idempotent → **battery-12 116 PASS** (`tests/v5.4.2-12/battery-12.sql`, `tally_app` end-to-end, two to four tenants) → `isolation-check-12.sql` **3 PASS** → race `races/pointer-mutex-12.sh` **PASS** → regressions 28 / 58 / 41 → **mutation: twenty-one guards broken one at a time (M1–M21), each caught by its named check** → mirror → image rebuilt, `tally-pg` recreated with a fresh volume, **zero error/fatal lines**, init file hash = committed tu.sql → **catalog parity**: tables 86 / views 6 / functions 407 / triggers 269 / policies 84 / FKs 369 / CHECKs 381 / EXCLUDE 11 / indexes 588 / FORCE RLS 83, identical on the patched clone and the build → on the build: batteries 28 / 58 / 41 / **116**, isolation 3, race PASS, patch re-applied over the build clean and battery-12 still 116 → `assert_tenant_isolation_invariants()` clean.

**Reviews (five rounds, Fable `general-purpose` + Opus `general-purpose`, hash-frozen: `75e33211` → `fd2418e2` → `ce8fcbe3` → `437ffb6b` → `3f74a76c`; Codex stalled in round 1 and was cancelled).** Round 1: tenant-movable cutover; unconstrained supersession (re-date, weaker basis, readings → reason); gate at month ends (48 cutover dates); back-dated "recorded" rows; equal-and-opposite filed inconclusive; FOR UPDATE / FOR SHARE lock strength and a deadlock; numeric scale; ENABLE ALWAYS. Round 2: self-promotion to platform_admin; migrated_full without readings; same-date parallel row by insertion order; cutover change under snapshot isolation; date-only failure superseded by a reason. Round 3: same-basis parallel row; users.id re-key; isolation refusal too broad. Round 4 (both "sound enough to mirror"): pointer from a stale RR snapshot; result-less row outranking a result; the round-3 guard refusing legitimate data. Round 5 (both "sound enough to mirror"): wording only — folded, re-verified, mirrored.
