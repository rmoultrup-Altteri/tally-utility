...
COMMENT ON FUNCTION public.enforce_tenant_cutover_date() IS
    'v5.4.2-12 (R-36). tenants.cutover_date is platform-set (tally_app may change it only as a platform administrator) and must satisfy max(migrated test_date) <= cutover_date <= min(recorded test_date), never NULL once history exists. Self-locks the tenant row FOR UPDATE on a change so it serialises with in-flight test inserts (which hold FOR KEY SHARE).';
...
BEGIN
    ...
    IF NEW.supersedes_test_id IS NOT NULL THEN
        ...
        -- recorded), and a test that carried readings may be corrected only by
        -- one that carries readings, so a derived failure cannot be replaced
        ...
        IF array_position(ARRAY['migrated_date_only', 'migrated_full', 'recorded'], NEW.record_basis)
           < array_position(ARRAY['migrated_date_only', 'migrated_full', 'recorded'], v_target.record_basis) THEN
            RAISE EXCEPTION 'meter test on meter %: a % row may not correct a % row — a correction may not weaken the record', NEW.meter_id, NEW.record_basis, v_target.record_basis
                USING ERRCODE = 'check_violation';
        END IF;
        IF v_target.has_readings AND NEW.load_results IS NULL THEN
            RAISE EXCEPTION 'meter test on meter %: test % carried readings, so its correction must carry readings — a derived result cannot be replaced by an asserted one or by an inconclusive_reason', NEW.meter_id, NEW.supersedes_test_id
                USING ERRCODE = 'check_violation';
        END IF;
    END IF;
...
--
-- R14. THE PLATFORM-ONLY RULE ON cutover_date NAMES tally_app. A future
--     second application role would be unwatched — the same shape as -11's
--     residual R7. Superuser and owner sessions (onboarding, migrations) may
--     set it.
--
