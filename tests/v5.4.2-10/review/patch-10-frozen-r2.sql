...
--        same tenant, and status = 'void' (round 1, Fable HIGH-2/3, Codex
--        MEDIUM-3): "correction" means "replaces a voided bill", and only a
--        void bill's period_end and snapshot are sealed (A-4 / CI-015) —
--        a live draft as the "original" let the resolver read a period_end
--        the attacker could move after the check, and let a deletable draft
--        snapshot anchor an original-world replay. At most ONE snapshotted
--        correction may replace a given bill on a given run (the target row
--        is the mutex; Fable LOW-7).
--
...
--      the target guard's own UPDATE waits behind them. The freeze guards
--      check EXISTS first (frozen → refuse with the frozen message) and only
--      then refuse a non-READ-COMMITTED transaction (Codex HIGH-1, Fable
--      LOW-8: the pin is needed — under REPEATABLE READ the ABSENCE of a
--      snapshot cannot be verified, a writer may have committed after this
--      transaction's snapshot — but it must not be the first thing a caller
--      hears; the contract is AC-31: edit these columns under READ COMMITTED).
--
...
--     (same tenant only) — customer-level correctness of target selection is
--     the engine's; stated boundary.
--
...
BEGIN
    ...
    IF current_setting('transaction_isolation') <> 'read committed' THEN
        RAISE EXCEPTION USING
            MESSAGE = format('correction target %s: its rate-date election may only be changed or removed under READ COMMITTED (v5.4.2-10, AC-31) — under %s the absence of a concurrently committed snapshot cannot be verified', OLD.id, current_setting('transaction_isolation')),
            ERRCODE = 'invalid_transaction_state';
    END IF;
...
COMMENT ON FUNCTION public.enforce_correction_target_frozen_under_snapshot() IS
    'v5.4.2-10 (A-3 follow-up). BEFORE UPDATE OF billing_run_id, voided_invoice_id, rate_date_mode, rate_date_override OR DELETE on correction_run_targets: refused while an invoice_calculation_snapshots row exists for the correction invoice (same run, replaces_invoice_id = voided_invoice_id) — its valid_at was checked against this election (EXISTS is checked first; a non-READ-COMMITTED transaction is then refused because it cannot verify absence — AC-31). The snapshot validator takes the target row''s lock via an UPDATE of updated_at, so this guard waits behind in-flight writers.';
...
BEGIN
    ...
    IF current_setting('transaction_isolation') <> 'read committed' THEN
        RAISE EXCEPTION USING
            MESSAGE = format('invoice %s: invoice_type / replaces_invoice_id / billing_run_id / period may only change under READ COMMITTED (v5.4.2-10, AC-31) — under %s the absence of a concurrently committed snapshot cannot be verified', OLD.invoice_number, current_setting('transaction_isolation')),
            ERRCODE = 'invalid_transaction_state';
    END IF;
    RETURN NEW;
END;
...
COMMENT ON FUNCTION public.enforce_invoice_snapshot_inputs() IS
    'v5.4.2-10 (A-3 follow-up). BEFORE INSERT OR UPDATE OF invoice_type, replaces_invoice_id, billing_run_id, period_start, period_end, created_at on invoices: INSERT stamps created_at = now() and it is write-once thereafter for non-superusers (the recorded_at lower bound for an off-run snapshot); UPDATE of the other five is refused while an invoice_calculation_snapshots row exists for the invoice (delete + re-snapshot, the -04 model), and — once no snapshot is visible — refused under any isolation level but READ COMMITTED (AC-31).';
...
BEGIN
    ...
    IF p_inv.invoice_type = 'correction' THEN
        ...
        IF EXISTS (
            SELECT 1
              FROM public.invoice_calculation_snapshots s2
              JOIN public.invoices i2 ON i2.id = s2.invoice_id
             WHERE i2.billing_run_id      = p_inv.billing_run_id
               AND i2.replaces_invoice_id = p_inv.replaces_invoice_id
               AND i2.invoice_type        = 'correction'
               AND i2.id                 <> p_inv.id) THEN
            RETURN format('another correction invoice on this run already carries a calculation snapshot replacing %s — one correction per replaced bill per run', v_orig.invoice_number);
        END IF;
...
COMMENT ON FUNCTION public.validate_calculation_snapshot() IS
    'CI-015 gate on invoice_calculation_snapshots INSERT (v5.4.2-04; coordinate binding added v5.4.2-10): parent must exist in the same tenant and be draft/held; billing_run_id equals the invoice''s; the (valid_at, recorded_at) pair is admissible per calculation_snapshot_coordinate_violation() — ordinary: valid_at = period_end, recorded_at within [run.started_at | invoice.created_at, now()]; correction: on a correction run with a correction_run_targets row, valid_at = get_correction_rate_date(), recorded_at within the run or equal to the replaced bill''s snapshot — with the run row held FOR KEY SHARE and the target row''s lock taken (UPDATE of updated_at); captured_at forced to now(); snapshot_schema_version must be one the function enumerates (v1) and every section must carry that version''s required keys (nested shapes for items/reads/line_items); period and customer match the invoice; line_items cites exactly the invoice''s lines.';
