...
--        (non-void) snapshotted correction may replace a given bill, on ANY
--        run (round 2, Fable MEDIUM-2: per-run scoping let a second correction
--        run rebill the same void bill) — the replaced bill's row is the mutex;
--        a voided correction may be corrected again.
--
...
--      target and then its run against a writer): invoice → replaced bill →
--      correction target → billing run.
--
    ...
    IF p_inv.invoice_type = 'correction' THEN
        ...
        IF EXISTS (
            SELECT 1
             ...
             WHERE i2.replaces_invoice_id = p_inv.replaces_invoice_id
               AND i2.invoice_type        = 'correction'
               AND i2.status             <> 'void'
               AND i2.id                 <> p_inv.id) THEN
            RETURN format('another live correction invoice already carries a calculation snapshot replacing %s — one live correction per replaced bill (void the earlier correction to correct again)', v_orig.invoice_number);
        END IF;
...
BEGIN
    ...
    -- Order: invoice → replaced bill → correction target → billing run (AC-31).
    ...
    IF v_inv.invoice_type = 'correction' AND v_inv.replaces_invoice_id IS NOT NULL THEN
        -- (2) the replaced (void) bill is the mutex for "one live correction per
        --     bill" across runs; A-4's void seal admits updated_at.
        UPDATE public.invoices o SET updated_at = now() WHERE o.id = v_inv.replaces_invoice_id;
