...
--        is the mutex; a voided correction may be corrected again.
--
...
--      first (round 3, Fable LOW-2: the reverse order deadlocked, retryably).
--
    ...
    IF p_inv.invoice_type = 'correction' THEN
        ...
        IF EXISTS (
            ...
            SELECT 1
             ...
             WHERE i2.invoice_type = 'correction'
               AND i2.status      <> 'void'
               AND i2.id          <> p_inv.id) THEN
            RETURN format('another live correction invoice already carries a calculation snapshot in the lineage of %s (any hop of the replaces_invoice_id chain) — one live correction per bill lineage (void the earlier correction to correct again)', v_orig.invoice_number);
        END IF;
