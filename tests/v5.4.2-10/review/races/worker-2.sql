...
VALUES ('00000000-0000-4000-8000-00000000fa01','00000000-0000-4000-8000-00000000f105','00000000-0000-4000-8000-00000000f003','2026-03-31',now(),'v1','race',
 ...
 '[{"invoice_line_item_id":"00000000-0000-4000-8000-00000000f205","charge_type":"base_charge","amount":12.50}]');
SELECT 'W2: snapshot inserted, holding run + target FOR SHARE for 5s' AS a, clock_timestamp();
UPDATE public.billing_runs SET total_invoices = total_invoices + 1, last_heartbeat_at = now() WHERE id = '00000000-0000-4000-8000-00000000f003';
SELECT 'W2: totals bumped' AS a, clock_timestamp();
SELECT pg_sleep(1);
COMMIT;
SELECT 'W2: committed' AS a, clock_timestamp();
