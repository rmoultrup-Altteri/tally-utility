-- Session A: inserts the correction's snapshot, holds the FOR SHARE locks on run+target
-- for a while (pg_sleep) before committing, so Session B can be observed blocking.
...
VALUES ('10000000-0000-4000-8000-00000000aa01', '10000000-0000-4000-8000-00000000c402', '10000000-0000-4000-8000-00000000b301',
   '2025-08-31', now(), 'v1', 'cdx10',
   ...
   (SELECT jsonb_build_array(jsonb_build_object('invoice_line_item_id', id, 'charge_type', 'base_charge', 'amount', 12.50)) FROM public.invoice_line_items WHERE invoice_id = '10000000-0000-4000-8000-00000000c402'));

\echo SESSION_A: snapshot inserted, holding locks for 6s
SELECT pg_sleep(6);
COMMIT;
