...
BEGIN
  ...
  -- The line NAMES ITS METER, as a real gas bill's usage line does. Without
  -- this the target's meter binding falls through to the premise-level
  -- fallback, and a sibling meter at the same address passes — which is
  -- correct behaviour for a bill that billed no meter, but it is not the
  -- shape being tested.
  INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id)
  VALUES ('00000000-0000-4000-8000-0000000012a1', p_id, 'gas', 'base_charge', 'Customer charge', p_amount,
          '00000000-0000-4000-8000-0000000012e1');
END $$;
...
DO $$ DECLARE d numeric; BEGIN
  ...
  -- Inflate the MONEY, which is the line item. Round 1: the first version of
  -- this test moved invoices.amount_due, which the gate no longer reads
  -- because nothing in the schema ties it to the lines — so it was testing a
  -- number that does not bill anybody.
  UPDATE public.invoice_line_items SET amount = 5000.00 WHERE invoice_id='00000000-0000-4000-8000-0000000012c8';
...
-- ============================================================================
-- GROUP J — the round-1 review findings, each pinned so it cannot come back
-- ============================================================================
RESET ROLE;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b1';

-- A fresh voided original for this group: Feb 2026, $100, meter M-1.
INSERT INTO public.billing_runs (id, tenant_id, run_number, billing_period, period_start, period_end, run_type, status, started_at) VALUES
  ('00000000-0000-4000-8000-0000000012ba','00000000-0000-4000-8000-0000000012a1','RUN-2026-02','2026-02','2026-02-01','2026-02-28','regular','in_progress','2026-03-01 09:00-05');
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012ca','INV-FEB','00000000-0000-4000-8000-0000000012ba','regular',NULL,'2026-02-01','2026-02-28',100.00);
SELECT pg_temp.snap('00000000-0000-4000-8000-0000000012ca','2026-02-28','2026-03-01 09:10-05');
UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012ca';
SELECT public.void_invoice('00000000-0000-4000-8000-0000000012ca','00000000-0000-4000-8000-0000000012b1','wrong_rate','group J fixture',true);

SET ROLE tally_app;

-- J1 (CRITICAL, both reviewers): the money is the LINE ITEMS. A bill whose
-- amount_due says 100 while its line item charges 5000 must not slip through
-- on the strength of the caller-set total.
INSERT INTO public.correction_run_targets (id, tenant_id, billing_run_id, voided_invoice_id, meter_id, customer_id, location_id, backbill_cause, anchor_date)
VALUES ('00000000-0000-4000-8000-0000000012da','00000000-0000-4000-8000-0000000012a1','00000000-0000-4000-8000-0000000012b4',
        '00000000-0000-4000-8000-0000000012ca','00000000-0000-4000-8000-0000000012e1','00000000-0000-4000-8000-0000000012c1',
        '00000000-0000-4000-8000-0000000012d1','non_registering_meter','2026-04-15');
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012cb','INV-CORR-FEB','00000000-0000-4000-8000-0000000012b4','correction','00000000-0000-4000-8000-0000000012ca','2026-02-01','2026-02-28',100.00);
UPDATE public.invoice_line_items SET amount = 5000.00 WHERE invoice_id='00000000-0000-4000-8000-0000000012cb';
DO $$ DECLARE e public.backbilling_period_evaluations%ROWTYPE; v_eid uuid; BEGIN
  v_eid := public.backbilling_evaluate_period('00000000-0000-4000-8000-0000000012da','00000000-0000-4000-8000-0000000012cb');
  SELECT * INTO e FROM public.backbilling_period_evaluations ev WHERE ev.id = v_eid;
  IF e.delta_amount <> 4900.00 OR e.direction <> 'customer_owes' THEN
    RAISE EXCEPTION 'J1 FAILED: evaluated delta % direction % — the evaluator is reading amount_due (100.00), not the 5000.00 line item', e.delta_amount, e.direction;
  END IF;
  RAISE NOTICE 'PASS J1: the delta comes from the LINE ITEMS (4900.00), not the caller-set amount_due (which says 0.00)';
END $$;

-- J2 (CRITICAL, Fable): a straddling period is refused outright rather than
-- billed at an amount the evidence itself calls partly forfeited.
DO $$ DECLARE e public.backbilling_period_evaluations%ROWTYPE; v_eid uuid; BEGIN
  -- Feb 2026 straddles the M-1 window, which opens 2026-01-10... it does not;
  -- use the January bill shape instead by moving the anchor so the window
  -- opens mid-period: months_from_anchor 3 from 2026-04-15 opens 2026-01-15.
  NULL;
END $$;
-- RESET ROLE first: a backdated started_at is the superuser migration
-- exemption, and under tally_app it is stamped now() (v5.4.2-10 F1).
RESET ROLE;
INSERT INTO public.billing_runs (id, tenant_id, run_number, billing_period, period_start, period_end, run_type, status, started_at) VALUES
  ('00000000-0000-4000-8000-0000000012bb','00000000-0000-4000-8000-0000000012a1','RUN-2026-01','2026-01','2026-01-01','2026-01-31','regular','in_progress','2026-02-01 09:00-05');
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012cc','INV-JAN','00000000-0000-4000-8000-0000000012bb','regular',NULL,'2026-01-01','2026-01-31',100.00);
SELECT pg_temp.snap('00000000-0000-4000-8000-0000000012cc','2026-01-31','2026-02-01 09:10-05');
UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012cc';
SELECT public.void_invoice('00000000-0000-4000-8000-0000000012cc','00000000-0000-4000-8000-0000000012b1','wrong_rate','group J straddle fixture',true);
SET ROLE tally_app;
INSERT INTO public.correction_run_targets (id, tenant_id, billing_run_id, voided_invoice_id, meter_id, customer_id, location_id, backbill_cause, anchor_date)
VALUES ('00000000-0000-4000-8000-0000000012db','00000000-0000-4000-8000-0000000012a1','00000000-0000-4000-8000-0000000012b4',
        '00000000-0000-4000-8000-0000000012cc','00000000-0000-4000-8000-0000000012e1','00000000-0000-4000-8000-0000000012c1',
        '00000000-0000-4000-8000-0000000012d1','meter_error','2026-06-15');
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012cd','INV-CORR-JAN','00000000-0000-4000-8000-0000000012b4','correction','00000000-0000-4000-8000-0000000012cc','2026-01-01','2026-01-31',131.00);
DO $$ DECLARE e public.backbilling_period_evaluations%ROWTYPE; v_eid uuid; BEGIN
  v_eid := public.backbilling_evaluate_period('00000000-0000-4000-8000-0000000012db','00000000-0000-4000-8000-0000000012cd');
  SELECT * INTO e FROM public.backbilling_period_evaluations ev WHERE ev.id = v_eid;
  IF NOT e.trimmed OR e.trimmed_amount <= 0 THEN
    RAISE EXCEPTION 'J2 FAILED: the January period straddles the 2026-01-10 window but was not recorded as trimmed (trimmed=% amount=%)', e.trimmed, e.trimmed_amount;
  END IF;
  RAISE NOTICE 'PASS J2a: the straddle is still RECORDED (% of % forfeited)', e.trimmed_amount, e.delta_amount;
  PERFORM pg_temp.snap('00000000-0000-4000-8000-0000000012cd','2026-01-31',now());
  BEGIN
    UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012cd';
    RAISE EXCEPTION 'J2b FAILED: issued at the full delta on evidence that calls part of it forfeited — the round-1 CRITICAL is back';
  EXCEPTION WHEN restrict_violation THEN
    RAISE NOTICE 'PASS J2b: a straddling period is REFUSED, not billed at an amount the evidence calls partly forfeited (Kyle Q-C pending)';
  END;
END $$;

-- J3 (CRITICAL, Codex): a "duplicate" that charges more than the bill it
-- replaces is a backbill whatever it is called.
RESET ROLE; SET ROLE tally_app;
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012ce','INV-DUP','00000000-0000-4000-8000-0000000012b4','duplicate','00000000-0000-4000-8000-0000000012ca','2026-02-01','2026-02-28',9999.00);
DO $$ BEGIN
  PERFORM pg_temp.snap('00000000-0000-4000-8000-0000000012ce','2026-02-28',now());
  UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012ce';
  RAISE EXCEPTION 'J3 FAILED: a duplicate charging 9999 for an already-billed period issued with no cap';
EXCEPTION WHEN restrict_violation THEN
  RAISE NOTICE 'PASS J3: a duplicate that charges MORE for an already-billed period is refused — the gate reads the act, not the type label';
END $$;

-- J4 (HIGH, Fable): void the bill, then rebill the same period as a plain
-- REGULAR invoice. No replaces_invoice_id at all.
INSERT INTO public.billing_runs (id, tenant_id, run_number, billing_period, period_start, period_end, run_type, status, started_at) VALUES
  ('00000000-0000-4000-8000-0000000012bc','00000000-0000-4000-8000-0000000012a1','RUN-2026-02-REDO','2026-02','2026-02-01','2026-02-28','regular','in_progress',now());
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012cf','INV-FEB-REDO','00000000-0000-4000-8000-0000000012bc','regular',NULL,'2026-02-01','2026-02-28',5000.00);
DO $$ BEGIN
  PERFORM pg_temp.snap('00000000-0000-4000-8000-0000000012cf','2026-02-28',now());
  UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012cf';
  RAISE EXCEPTION 'J4 FAILED: a plain regular rebill of the voided period issued uncapped';
EXCEPTION WHEN restrict_violation THEN
  RAISE NOTICE 'PASS J4: void-then-rebill-as-regular is refused — being a rebill is a fact about the premise and period, not about replaces_invoice_id';
END $$;

-- J4b: and a rebill that does NOT charge more still passes. The cap is on the
-- increase; refusing a reduction would be a guard inventing a duty.
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012e0','INV-FEB-LOWER','00000000-0000-4000-8000-0000000012bc','regular',NULL,'2026-02-01','2026-02-28',80.00);
DO $$ DECLARE st text; BEGIN
  PERFORM pg_temp.snap('00000000-0000-4000-8000-0000000012e0','2026-02-28',now());
  UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012e0';
  SELECT status INTO st FROM public.invoices WHERE id='00000000-0000-4000-8000-0000000012e0';
  IF st <> 'pending' THEN RAISE EXCEPTION 'J4b FAILED: status %', st; END IF;
  RAISE NOTICE 'PASS J4b: a rebill that charges LESS still issues — the cap bounds the increase, not every re-issue';
END $$;

-- J5 (HIGH, Fable): the target must describe the bill it corrects.
-- Its own correction run: correction_run_targets is UNIQUE on
-- (billing_run_id, voided_invoice_id), which is what makes gate (iii)'s
-- target lookup deterministic, so J1's target already holds (b4, ca).
RESET ROLE;
INSERT INTO public.billing_runs (id, tenant_id, run_number, billing_period, period_start, period_end, run_type, status, correction_rate_mode, started_at) VALUES
  ('00000000-0000-4000-8000-0000000012bd','00000000-0000-4000-8000-0000000012a1','RUN-CORR-J5','2026-06','2026-06-01','2026-06-30','correction','in_progress','historical',now());
SET ROLE tally_app;
DO $$ BEGIN
  INSERT INTO public.correction_run_targets (tenant_id, billing_run_id, voided_invoice_id, meter_id, customer_id, location_id, backbill_cause, anchor_date)
  VALUES ('00000000-0000-4000-8000-0000000012a1','00000000-0000-4000-8000-0000000012bd','00000000-0000-4000-8000-0000000012ca',
          '00000000-0000-4000-8000-0000000012e2','00000000-0000-4000-8000-0000000012c1','00000000-0000-4000-8000-0000000012d1',
          'meter_error','2026-06-15');
  RAISE EXCEPTION 'J5 FAILED: a target named a meter the voided bill never billed — and that meter''s test date sets the window';
EXCEPTION WHEN restrict_violation THEN
  RAISE NOTICE 'PASS J5a: the target''s meter must be one the voided bill actually charged for';
END $$;
DO $$ BEGIN
  INSERT INTO public.correction_run_targets (tenant_id, billing_run_id, voided_invoice_id, meter_id, customer_id, location_id, backbill_cause, anchor_date)
  VALUES ('00000000-0000-4000-8000-0000000012a1','00000000-0000-4000-8000-0000000012bd','00000000-0000-4000-8000-0000000012ca',
          '00000000-0000-4000-8000-0000000012e1','00000000-0000-4000-8000-0000000012c2','00000000-0000-4000-8000-0000000012d1',
          'meter_error','2026-06-15');
  RAISE EXCEPTION 'J5b FAILED: a target named a different customer — and the customer decides the protected class';
EXCEPTION WHEN restrict_violation THEN
  RAISE NOTICE 'PASS J5b: the target''s customer must be the bill''s own customer (this is what uncapped a residential bill under explicit_class)';
END $$;

-- J6: the evidence row records the meter and the test date that produced the
-- window, so the trim is re-derivable when last_test_date has since moved.
DO $$ DECLARE e public.backbilling_period_evaluations%ROWTYPE; BEGIN
  SELECT * INTO e FROM public.backbilling_period_evaluations ev
   WHERE ev.target_id='00000000-0000-4000-8000-0000000012db' ORDER BY ev.evaluation_seq DESC LIMIT 1;
  IF e.meter_id IS NULL OR e.last_test_date_used IS NULL THEN
    RAISE EXCEPTION 'J6 FAILED: the evidence row does not record the meter (%) or the test date (%) that produced its window', e.meter_id, e.last_test_date_used;
  END IF;
  RAISE NOTICE 'PASS J6: the evidence row names the meter and the test date it used — last_test_date is mutable, so without these the window is not re-derivable';
END $$;


-- J7: BOTH numbers are the money, and a backbill must say the same thing to
-- the bill and to the ledger. The line items are (6)(B)(v)'s itemised
-- content; amount_due is what posts to the ledger (void_invoice reverses
-- exactly -(amount_due)). Round 1 found the draft resting on amount_due
-- alone; the first round-2 fix then rested on the lines alone, which is the
-- same defect mirrored — honest $105 of lines with amount_due at $5,000
-- issued clean and the ledger would have posted $5,000.
RESET ROLE; SET LOCAL app.user_id='00000000-0000-4000-8000-0000000012b1';
INSERT INTO public.billing_runs (id, tenant_id, run_number, billing_period, period_start, period_end, run_type, status, started_at) VALUES
  ('00000000-0000-4000-8000-0000000012be','00000000-0000-4000-8000-0000000012a1','RUN-2026-03','2026-03','2026-03-01','2026-03-31','regular','in_progress','2026-04-01 09:00-05');
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012e5','INV-MAR','00000000-0000-4000-8000-0000000012be','regular',NULL,'2026-03-01','2026-03-31',100.00);
SELECT pg_temp.snap('00000000-0000-4000-8000-0000000012e5','2026-03-31','2026-04-01 09:10-05');
UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012e5';
SELECT public.void_invoice('00000000-0000-4000-8000-0000000012e5','00000000-0000-4000-8000-0000000012b1','wrong_rate','J7 fixture',true);
SET ROLE tally_app;
INSERT INTO public.correction_run_targets (id, tenant_id, billing_run_id, voided_invoice_id, meter_id, customer_id, location_id, backbill_cause, anchor_date)
VALUES ('00000000-0000-4000-8000-0000000012de','00000000-0000-4000-8000-0000000012a1','00000000-0000-4000-8000-0000000012b4',
        '00000000-0000-4000-8000-0000000012e5','00000000-0000-4000-8000-0000000012e1','00000000-0000-4000-8000-0000000012c1',
        '00000000-0000-4000-8000-0000000012d1','rate_misapplication','2026-06-15');
SELECT pg_temp.inv('00000000-0000-4000-8000-0000000012e6','INV-CORR-MAR','00000000-0000-4000-8000-0000000012b4','correction','00000000-0000-4000-8000-0000000012e5','2026-03-01','2026-03-31',105.00);
DO $$ BEGIN
  PERFORM public.backbilling_evaluate_period('00000000-0000-4000-8000-0000000012de','00000000-0000-4000-8000-0000000012e6');
  UPDATE public.invoices SET amount_due = 5000.00 WHERE id='00000000-0000-4000-8000-0000000012e6';
  PERFORM pg_temp.snap('00000000-0000-4000-8000-0000000012e6','2026-03-31',now());
  BEGIN
    UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012e6';
    RAISE EXCEPTION 'J7 FAILED: issued with honest lines but an inflated amount_due — the ledger would post 5000.00';
  EXCEPTION WHEN restrict_violation THEN
    RAISE NOTICE 'PASS J7a: the LEDGER path is closed too — inflating amount_due raises the measured charge above the evidence, so it is refused';
  END;
END $$;
DO $$ DECLARE st text; BEGIN
  UPDATE public.invoices SET amount_due = 105.00 WHERE id='00000000-0000-4000-8000-0000000012e6';
  UPDATE public.invoices SET status='pending' WHERE id='00000000-0000-4000-8000-0000000012e6';
  SELECT status INTO st FROM public.invoices WHERE id='00000000-0000-4000-8000-0000000012e6';
  IF st <> 'pending' THEN RAISE EXCEPTION 'J7b FAILED: status %', st; END IF;
  RAISE NOTICE 'PASS J7b: with the two totals agreeing and the evaluation current, the correction issues';
END $$;
