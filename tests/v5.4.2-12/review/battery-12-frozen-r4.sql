...
DO $$ BEGIN
  -- M-7's migrated_full 2025-11-01 found the meter 5% fast. File a PARALLEL
  -- date-only "accurate" row for the same date, not superseding it.
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome)
  ...
  RAISE EXCEPTION 'K5 FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%would displace it%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS K5: a parallel date-only "accurate" row beside a derived failure is refused (rounds 2-3)';
END $$;
DO $$ BEGIN
  -- M-3's recorded 2026-05-01 test is a derived FAST. A parallel recorded row
  -- with only a reason, same basis, no supersession.
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
        performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, inconclusive_reason)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e3', DATE '2026-05-01', 'periodic', 'recorded',
        'A. Tester', 'prover', 'SN-3', 1.0, 'second look, unreliable');
  RAISE EXCEPTION 'K5b FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%would displace it%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS K5b: a same-basis parallel row with only a reason beside a derived failure is refused (round 3, both reviewers)';
END $$;
DO $$
...
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%would displace it%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS K6b: nor displaced by a parallel date-only "accurate" row (round 3)';
END $$;
DO $$ BEGIN
