...
INSERT INTO public.users (id, tenant_id, display_name, email, role) VALUES
  ...
  ('00000000-0000-4000-8000-0000000012b9', '00000000-0000-4000-8000-0000000012a1', 'Platform', 'pa@bat12.test', 'platform_admin');
...
BEGIN
  ...
  SELECT outcome, found_defective INTO r FROM public.meter_tests WHERE id = v;
  IF r.outcome <> 'inconclusive' OR NOT r.found_defective THEN RAISE EXCEPTION 'A6 FAILED: +3/-3 gave % defective=%', r.outcome, r.found_defective; END IF;
  RAISE NOTICE 'PASS A6: equal and opposite deviations beyond the threshold: no direction (inconclusive) but found_defective — the meter failed (round 1)';
END $$;
...
DO $$ BEGIN
  PERFORM pg_temp.rec('00000000-0000-4000-8000-0000000012e7', DATE '2026-02-10', '[{"load_point":"check","standard_volume":100,"meter_volume":101,"notes":{"x":[1,2]}}]'::jsonb);
  RAISE EXCEPTION 'A10g FAILED';
EXCEPTION WHEN invalid_parameter_value THEN RAISE NOTICE 'PASS A10g: a non-string load note is refused'; END $$;
DO $$
DECLARE v uuid; a numeric; b numeric;
BEGIN
  v := pg_temp.rec('00000000-0000-4000-8000-0000000012e7', DATE '2026-02-15',
                   '[{"load_point":"check","standard_volume":0.0003000000,"meter_volume":100000}]'::jsonb);
  SELECT max_abs_error_pct INTO a FROM public.meter_tests WHERE id = v;
  SELECT max(abs(error_pct)) INTO b FROM public.meter_test_load_results WHERE meter_test_id = v;
  IF a IS DISTINCT FROM b THEN RAISE EXCEPTION 'A16 FAILED: stored max % vs load rows %', a, b; END IF;
  RAISE NOTICE 'PASS A16: trailing zeros beyond four decimals do not change the derived maximum — it equals the load rows'' error exactly (round 1)';
END $$;
...
BEGIN
  ...
  v := pg_temp.rec('00000000-0000-4000-8000-0000000012e1', DATE '2026-01-20', jsonb_build_array(pg_temp.ld('check', 100, 104)));
  INSERT INTO bat12_ids VALUES ('m1_0120', v);
  SELECT last_test_date, last_test_result INTO m FROM public.meters WHERE id = '00000000-0000-4000-8000-0000000012e1';
  ...
  -- a same-date correction of a mis-keyed reading: the corrected row
  -- replaces the original, the pointer stays on the date.
  v := pg_temp.rec('00000000-0000-4000-8000-0000000012e1', DATE '2026-06-01', jsonb_build_array(pg_temp.ld('check', 100, 97)),
                   'periodic', (SELECT id FROM bat12_ids WHERE k = 'm1_0601'), 'meter volume keyed as 96, sheet says 97');
  INSERT INTO bat12_ids VALUES ('m1_0601c', v);
  SELECT last_test_date, last_test_result INTO m FROM public.meters WHERE id = '00000000-0000-4000-8000-0000000012e1';
  IF m.last_test_date <> DATE '2026-06-01' OR m.last_test_result <> 'failed' THEN
    RAISE EXCEPTION 'C4 FAILED: pointer %/%', m.last_test_date, m.last_test_result; END IF;
  RAISE NOTICE 'PASS C4: a same-date correction with corrected readings supersedes the original; the pointer stays on 2026-06-01';
END $$;
-- C5 (round 1, both reviewers): what a correction may NOT do.
DO $$ BEGIN
  PERFORM pg_temp.rec('00000000-0000-4000-8000-0000000012e1', DATE '2026-02-01', jsonb_build_array(pg_temp.ld('check', 100, 100)),
                      'periodic', (SELECT id FROM bat12_ids WHERE k = 'm1_0510'), 'it was really February');
  RAISE EXCEPTION 'C5 FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%may not move a test''s date%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS C5: a correction may not move a test earlier (it would pull window_start back three months)';
END $$;
DO $$ BEGIN
  PERFORM pg_temp.rec('00000000-0000-4000-8000-0000000012e1', DATE '2026-09-20', jsonb_build_array(pg_temp.ld('check', 100, 100)),
                      'periodic', (SELECT id FROM bat12_ids WHERE k = 'm1_0510'), 'it was really September');
  RAISE EXCEPTION 'C5b FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%may not move a test''s date%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS C5b: nor later (past an anchor it would drop the test and leave only the six-month cap)';
END $$;
DO $$ BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
        performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, inconclusive_reason, supersedes_test_id, supersede_reason)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e1', DATE '2026-05-10', 'periodic', 'recorded',
        'A. Tester', 'prover', 'SN-1', 1.0, 'on reflection, unreliable', (SELECT id FROM bat12_ids WHERE k = 'm1_0510'), 'reclassify');
  RAISE EXCEPTION 'C5c FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%must carry readings%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS C5c: a test with readings cannot be "corrected" into a bare inconclusive_reason';
END $$;
DO $$
DECLARE v uuid;
BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
        performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, load_results)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e7', DATE '2025-11-01', 'periodic', 'migrated_full',
        'Legacy Tester', 'prover', 'SN-7', 1.0, jsonb_build_array(pg_temp.ld('check', 100, 105)))
  RETURNING id INTO v;
  INSERT INTO bat12_ids VALUES ('m7_1101', v);
END $$;
DO $$ BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome, supersedes_test_id, supersede_reason)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e7', DATE '2025-11-01', 'periodic', 'migrated_date_only',
          'accurate', (SELECT id FROM bat12_ids WHERE k = 'm7_1101'), 'legacy system says it passed');
  RAISE EXCEPTION 'C5d FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%may not weaken the record%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS C5d: a date-only row asserting "accurate" cannot supersede a full record whose readings found it 5%% fast';
END $$;
DO $$ BEGIN
...
DO $$ BEGIN
  PERFORM pg_temp.rec('00000000-0000-4000-8000-0000000012e1', DATE '2026-02-01', jsonb_build_array(pg_temp.ld('check', 100, 96)),
                      'periodic', (SELECT id FROM public.meter_tests WHERE meter_id = '00000000-0000-4000-8000-0000000012e7' AND test_date = DATE '2026-02-01'), 'wrong meter');
  RAISE EXCEPTION 'C6b FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%same meter%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS C6b: a test may not supersede another meter''s test';
END $$;
DO $$ BEGIN
  PERFORM pg_temp.rec('00000000-0000-4000-8000-0000000012e1', DATE '2026-05-10', jsonb_build_array(pg_temp.ld('check', 100, 96)),
...
DO $$ BEGIN
  ...
  RAISE EXCEPTION 'E0 FAILED';
EXCEPTION WHEN insufficient_privilege THEN RAISE NOTICE 'PASS E0: the tenant cannot set its own cutover_date — it decides when R-36''s gate lapses (round 1, both reviewers)'; END $$;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b9';
UPDATE public.tenants SET cutover_date = DATE '2026-02-01' WHERE id = '00000000-0000-4000-8000-0000000012a2';
DO $$
DECLARE n int;
BEGIN
  SELECT count(*) INTO n FROM public.tenant_configuration_history
   WHERE tenant_id = '00000000-0000-4000-8000-0000000012a2' AND config_key = 'cutover_date' AND new_value = '"2026-02-01"'::jsonb
     AND changed_by = '00000000-0000-4000-8000-0000000012b9';
  IF n <> 1 THEN RAISE EXCEPTION 'E0b FAILED: % history rows', n; END IF;
  RAISE NOTICE 'PASS E0b: the platform administrator sets it, and the change is logged in tenant_configuration_history with who made it';
END $$;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b2';
DO $$ BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis)
...
END $$;
-- E6/E7 on T1 (cutover 2026-01-15, recorded tests from 2026-01-20).
DO $$ BEGIN
  UPDATE public.tenants SET cutover_date = DATE '2026-03-01' WHERE id = '00000000-0000-4000-8000-0000000012a1';
  RAISE EXCEPTION 'E7 FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%after a recorded meter test%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS E7: cutover cannot move past a recorded test — recorded history stays on or after go-live';
END $$;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b1';
DO $$ BEGIN
  PERFORM pg_temp.rec('00000000-0000-4000-8000-0000000012e6', DATE '2026-01-10', jsonb_build_array(pg_temp.ld('check', 100, 100)));
  RAISE EXCEPTION 'E6 FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%recorded row must be dated on or after%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS E6: a "recorded" test back-dated before go-live is refused — it would escape the migrated-provenance gate (round 1)';
END $$;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b2';
DO $$ BEGIN
...
-- F8: supersession — on M-1 the original 06-01 row is superseded by a
-- same-date correction, which is what governs an anchor of 06-15.
DO $$
DECLARE g record;
BEGIN
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e1', DATE '2026-06-15');
  IF g.meter_test_id IS DISTINCT FROM (SELECT id FROM bat12_ids WHERE k = 'm1_0601c') THEN
    RAISE EXCEPTION 'F8 FAILED: governing % on %', g.meter_test_id, g.test_date; END IF;
  RAISE NOTICE 'PASS F8: the superseded 06-01 row is excluded — the governing test for 06-15 is its same-date correction';
END $$;
...
INSERT INTO public.tenants (id, name, slug, cutover_date) VALUES
  ...
  ('00000000-0000-4000-8000-0000000012a4', 'T4 Gasco', 'bat12-t4', DATE '2026-08-31');
INSERT INTO public.customers (id, tenant_id, customer_number) VALUES
  ('00000000-0000-4000-8000-0000000012c3', '00000000-0000-4000-8000-0000000012a3', 'BAT12-C3'),
  ('00000000-0000-4000-8000-0000000012c4', '00000000-0000-4000-8000-0000000012a4', 'BAT12-C4');
INSERT INTO public.service_locations (id, tenant_id, customer_id, location_number, address_line1, city, state, zip) VALUES
  ('00000000-0000-4000-8000-0000000012d7', '00000000-0000-4000-8000-0000000012a3', '00000000-0000-4000-8000-0000000012c3', 'LOC-7', '7 Main', 'Austin', 'TX', '78707'),
  ('00000000-0000-4000-8000-0000000012d8', '00000000-0000-4000-8000-0000000012a4', '00000000-0000-4000-8000-0000000012c4', 'LOC-8', '8 Main', 'Austin', 'TX', '78708');
INSERT INTO public.meters (id, tenant_id, meter_number, location_id, service_type) VALUES
  ('00000000-0000-4000-8000-0000000012e9', '00000000-0000-4000-8000-0000000012a3', 'M-9', '00000000-0000-4000-8000-0000000012d7', 'gas'),
  ('00000000-0000-4000-8000-0000000012ea', '00000000-0000-4000-8000-0000000012a4', 'M-10', '00000000-0000-4000-8000-0000000012d8', 'gas');
INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis) VALUES
  ('00000000-0000-4000-8000-0000000012a3', '00000000-0000-4000-8000-0000000012e9', DATE '2026-03-31', 'periodic', 'migrated_date_only'),
  ('00000000-0000-4000-8000-0000000012a4', '00000000-0000-4000-8000-0000000012ea', DATE '2026-08-31', 'periodic', 'migrated_date_only');
DO $$
...
-- ======================================================================= K
-- Review round 2. (The READ COMMITTED requirement on a cutover change cannot
-- be exercised inside this one READ COMMITTED transaction; it is checked by
-- tests/v5.4.2-12/isolation-check-12.sql.)
SET ROLE tally_app;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b1';
DO $$ BEGIN
  UPDATE public.users SET role = 'platform_admin' WHERE id = '00000000-0000-4000-8000-0000000012b1';
  RAISE EXCEPTION 'K1 FAILED';
EXCEPTION WHEN insufficient_privilege THEN RAISE NOTICE 'PASS K1: an operator cannot promote itself to platform_admin (round 2, both reviewers)'; END $$;
DO $$ BEGIN
  INSERT INTO public.users (id, tenant_id, display_name, email, role)
  VALUES ('00000000-0000-4000-8000-0000000012bb', '00000000-0000-4000-8000-0000000012a1', 'Sock', 'sock@bat12.test', 'platform_admin');
  RAISE EXCEPTION 'K2 FAILED';
EXCEPTION WHEN insufficient_privilege THEN RAISE NOTICE 'PASS K2: an operator cannot insert a new platform_admin user'; END $$;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b9';
DO $$ BEGIN
  INSERT INTO public.users (id, tenant_id, display_name, email, role)
  VALUES ('00000000-0000-4000-8000-0000000012bc', '00000000-0000-4000-8000-0000000012a1', 'Admin2', 'a2@bat12.test', 'platform_admin');
  UPDATE public.users SET display_name = 'Platform (renamed)' WHERE id = '00000000-0000-4000-8000-0000000012b9';
  RAISE NOTICE 'PASS K3: a platform administrator may grant the role, and an administrator''s own row stays editable';
END $$;
SET LOCAL app.user_id = '00000000-0000-4000-8000-0000000012b1';
DO $$ BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
        performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, inconclusive_reason)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e6', DATE '2005-01-01', 'periodic', 'migrated_full',
        'x', 'x', 'x', 1, 'legacy record has no readings');
  RAISE EXCEPTION 'K4 FAILED';
EXCEPTION WHEN check_violation THEN RAISE NOTICE 'PASS K4: a migrated_full row without readings is refused — the label alone would switch the R-36 gate off (round 2)'; END $$;
DO $$
DECLARE g record;
BEGIN
  -- M-7's migrated_full 2025-11-01 found the meter 5% fast. A PARALLEL
  -- date-only "accurate" row for the same date, not superseding it, is
  -- accepted (round 4: two legacy sources for one day are legitimate) — and
  -- the derived failure still governs.
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e7', DATE '2025-11-01', 'periodic', 'migrated_date_only', 'accurate');
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e7', DATE '2025-11-02');
  IF g.meter_test_id IS DISTINCT FROM (SELECT id FROM bat12_ids WHERE k = 'm7_1101') OR NOT g.prior_test_failed THEN
    RAISE EXCEPTION 'K5 FAILED: governing % basis % failed=%', g.meter_test_id, g.record_basis, g.prior_test_failed; END IF;
  RAISE NOTICE 'PASS K5: a parallel date-only "accurate" row beside a derived failure is accepted but does not displace it (rounds 2-4)';
END $$;
DO $$
DECLARE g record; r text;
BEGIN
  -- M-3's recorded 2026-05-01 test is a derived FAST. A parallel recorded
  -- row with only a reason (a second attempt that produced no measurement).
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
        performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, inconclusive_reason)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e3', DATE '2026-05-01', 'periodic', 'recorded',
        'A. Tester', 'prover', 'SN-3', 1.0, 'second attempt, prover lost pressure');
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e3', DATE '2026-05-02');
  SELECT last_test_result INTO r FROM public.meters WHERE id = '00000000-0000-4000-8000-0000000012e3';
  IF g.outcome <> 'fast' OR NOT g.prior_test_failed OR r <> 'failed' THEN
    RAISE EXCEPTION 'K5b FAILED: governing % failed=% pointer %', g.outcome, g.prior_test_failed, r; END IF;
  RAISE NOTICE 'PASS K5b: a same-day reason-only attempt beside a derived failure is recorded, and the failure still governs both readers (rounds 3-4)';
END $$;
DO $$
DECLARE v uuid; g record;
BEGIN
  -- A same-day RETEST with readings (as-left after adjustment) is accepted
  -- and governs.
  v := pg_temp.rec('00000000-0000-4000-8000-0000000012e3', DATE '2026-05-01', jsonb_build_array(pg_temp.ld('as_left', 100, 100.5)));
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e3', DATE '2026-05-02');
  IF g.meter_test_id IS DISTINCT FROM v OR g.outcome <> 'accurate' THEN RAISE EXCEPTION 'K5c FAILED: % %', g.meter_test_id, g.outcome; END IF;
  RAISE NOTICE 'PASS K5c: a same-day retest WITH readings is accepted and governs (as-found / as-left)';
END $$;
DO $$
DECLARE v uuid;
BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e4', DATE '2026-01-15', 'periodic', 'migrated_date_only', 'fast')
  RETURNING id INTO v;
  INSERT INTO bat12_ids VALUES ('m4_0115', v);
END $$;
DO $$ BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
        performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, inconclusive_reason, supersedes_test_id, supersede_reason)
  VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e4', DATE '2026-01-15', 'periodic', 'recorded',
        'A. Tester', 'prover', 'SN-4', 1.0, 'cannot confirm legacy result', (SELECT id FROM bat12_ids WHERE k = 'm4_0115'), 'legacy fast unverified');
  RAISE EXCEPTION 'K6 FAILED';
EXCEPTION WHEN check_violation THEN
  IF SQLERRM NOT LIKE '%found the meter defective%' THEN RAISE; END IF;
  RAISE NOTICE 'PASS K6: a date-only failure cannot be superseded by a bare inconclusive_reason on the cutover day (round 2)';
END $$;
DO $$ BEGIN
  INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome)
...
END $$;
DO $$ BEGIN
  -- A platform_admin row that nothing references (Admin2, created in K3)
  -- lives in T1. In ONE statement, move
  -- my row aside and give the admin row my id: rows are visited in insertion
  -- order (U1 first), so the primary key never collides, and every foreign
  -- key to my id still resolves at statement end — the session is then the
  -- administrator without the role column moving.
  UPDATE public.users SET id = CASE id WHEN '00000000-0000-4000-8000-0000000012b1'::uuid THEN '00000000-0000-4000-8000-0000000012be'::uuid
                                        ELSE '00000000-0000-4000-8000-0000000012b1'::uuid END
   WHERE id IN ('00000000-0000-4000-8000-0000000012b1', '00000000-0000-4000-8000-0000000012bc');
  IF public.is_platform_admin() THEN RAISE EXCEPTION 'K7 FAILED: the session became platform_admin by re-keying'; END IF;
  RAISE EXCEPTION 'K7 FAILED';
EXCEPTION WHEN insufficient_privilege THEN RAISE NOTICE 'PASS K7: a user''s id cannot be re-keyed — swapping ids with the platform_admin row would transfer the role (round 3, Opus)'; END $$;
RESET ROLE;
-- K8: the readers' ranking, independent of the insert guard. As owner, with
-- the guard disabled inside a savepoint, plant a weaker same-date row beside
-- M-4's asserted failure (date-only "fast", 2026-01-15) and read both readers.
SAVEPOINT k8;
ALTER TABLE public.meter_tests DISABLE TRIGGER a_enforce_meter_test_record;
INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome, found_defective)
VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e4', DATE '2026-01-15', 'periodic', 'migrated_date_only', 'accurate', false);
DO $$
DECLARE g record; r text;
BEGIN
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e4', DATE '2026-01-16');
  SELECT last_test_result INTO r FROM public.meters WHERE id = '00000000-0000-4000-8000-0000000012e4';
  IF g.meter_test_id IS DISTINCT FROM (SELECT id FROM bat12_ids WHERE k = 'm4_0115') OR r <> 'failed' THEN
    RAISE EXCEPTION 'K8 FAILED: governing % outcome %, pointer %', g.meter_test_id, g.outcome, r; END IF;
  RAISE NOTICE 'PASS K8: with a later same-date "accurate" assertion on disk, both readers still rank the asserted failure first (round 3 ranking)';
END $$;
-- K8b: between two rows without readings and without a failure, the
-- stronger record governs — M-7's recorded inconclusive 2026-02-11 (A11)
-- against a later-planted date-only "accurate" on the same date.
INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome, found_defective)
VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e7', DATE '2026-02-11', 'periodic', 'migrated_date_only', 'accurate', false);
DO $$
DECLARE g record;
BEGIN
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e7', DATE '2026-02-12');
  IF g.record_basis <> 'recorded' OR g.outcome <> 'inconclusive' THEN
    RAISE EXCEPTION 'K8b FAILED: governing % %', g.record_basis, g.outcome; END IF;
  RAISE NOTICE 'PASS K8b: between assertions with no failure, the stronger record governs, not the later insertion (round 2 ranking)';
END $$;
-- K8c: a row with readings beats a later same-basis row without —
-- M-1's recorded 2026-05-10 (readings, accurate) against a planted recorded
-- inconclusive_reason row on the same date.
INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis,
      performed_by_name, test_equipment, meter_serial_at_test, multiplier_at_test, inconclusive_reason, outcome)
VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e1', DATE '2026-05-10', 'periodic', 'recorded',
      'A. Tester', 'prover', 'SN-1', 1.0, 'planted', 'inconclusive');
DO $$
DECLARE g record;
BEGIN
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e1', DATE '2026-05-11');
  IF g.meter_test_id IS DISTINCT FROM (SELECT id FROM bat12_ids WHERE k = 'm1_0510') THEN
    RAISE EXCEPTION 'K8c FAILED: governing % %', g.record_basis, g.outcome; END IF;
  RAISE NOTICE 'PASS K8c: a row with readings governs over a later same-basis row without (round 3 ranking)';
END $$;
-- K8d (round 4): a row with a result outranks a later one with none — on
-- M-6 (no history), plant date-only "accurate", then date-only NULL.
INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis, outcome, found_defective)
VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e6', DATE '2025-09-01', 'periodic', 'migrated_date_only', 'accurate', false);
INSERT INTO public.meter_tests (tenant_id, meter_id, test_date, test_kind, record_basis)
VALUES ('00000000-0000-4000-8000-0000000012a1', '00000000-0000-4000-8000-0000000012e6', DATE '2025-09-01', 'periodic', 'migrated_date_only');
DO $$
DECLARE g record;
BEGIN
  SELECT * INTO g FROM public.meter_governing_test('00000000-0000-4000-8000-0000000012e6', DATE '2025-09-02');
  IF g.outcome IS DISTINCT FROM 'accurate' THEN RAISE EXCEPTION 'K8d FAILED: governing outcome %', g.outcome; END IF;
  RAISE NOTICE 'PASS K8d: a same-date row with no result does not outrank one with a result (round 4 ranking)';
END $$;
ROLLBACK TO SAVEPOINT k8;
...
BEGIN
  SELECT count(*) INTO n FROM pg_trigger
   WHERE NOT tgisinternal AND tgenabled <> 'A'
     AND tgname IN ('a_enforce_tenant_cutover_date', 'a_enforce_meter_test_pointer', 'a_enforce_meter_test_record',
                    'z_meter_test_after_record', 'append_only', 'no_truncate', 'a_enforce_load_results_written_by_test',
                    'a_enforce_meter_test_absence_declaration', 'z_meter_test_absence_after_declare')
     AND tgrelid IN ('public.tenants'::regclass, 'public.meters'::regclass, 'public.meter_tests'::regclass,
                     'public.meter_test_load_results'::regclass, 'public.meter_test_absence_declarations'::regclass)
     AND NOT (tgrelid = 'public.meters'::regclass AND tgname IN ('no_truncate'));
  SELECT n + count(*) INTO n FROM pg_trigger
   WHERE NOT tgisinternal AND tgenabled <> 'A'
     AND ((tgrelid = 'public.users'::regclass AND tgname = 'a_enforce_platform_admin_grant')
       OR (tgrelid = 'public.tenants'::regclass AND tgname = 'trg_record_tenant_configuration_change'));
  IF n <> 0 THEN RAISE EXCEPTION 'I2 FAILED: % patch trigger(s) not ENABLE ALWAYS', n; END IF;
  RAISE NOTICE 'PASS I2: every trigger this patch creates is ENABLE ALWAYS (round 1: the cutover guard was not)';
END $$;
