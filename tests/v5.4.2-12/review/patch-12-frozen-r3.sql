...
COMMENT ON FUNCTION public.enforce_platform_admin_grant() IS
    'v5.4.2-12 (review round 2). For tally_app, a users row may take the platform_admin role only when the session is already a platform administrator. Keeping or dropping the role is unaffected; the owner is unaffected. The one role invariant the platform-only cutover_date rests on — not the role model Kyle''s coda item A2 asks for.';
...
--   * It must run under READ COMMITTED. Test inserts only KEY-SHARE the tenant
--     row and never modify it, so a REPEATABLE READ or SERIALIZABLE change
--     whose snapshot predates a committed insert gets no serialisation error
--     and its bounds check cannot see that row — the invariant broke in
--     three measured interleavings. Refused rather than documented.
...
BEGIN
    ...
    -- tie of dates the STRONGER record wins, then the later one (review round
    -- 2): otherwise a parallel date-only row, filed beside a derived failure
    -- without superseding it, would take over by insertion order and do what
...
--
-- Two tests on the same date: the stronger record_basis governs, then the
-- later insertion (review round 2) — the same order the pointer uses.
--
...
--     operator and viewer remain freely assignable within a tenant, and the
--     permission model (Kyle's coda A2; "Session D") is still open.
--
