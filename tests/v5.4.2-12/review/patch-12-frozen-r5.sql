...
--
-- Review round 4 (Fable + Opus, frozen 437ffb6b): both "sound enough to
-- mirror"; their measured LOW fixes folded here:
--   * Both: a test insert under REPEATABLE READ refreshed the pointer from a
--     stale snapshot when a concurrent insert had not changed the pointer.
--     The pointer is now written on every insert (row-version mutex), so the
--     snapshot session fails with a serialisation error instead.
--   * Fable: a same-date row with no result outranked one with a result.
--     Ranked below it now.
--   * Both: round 3's insert guard refused legitimate rows the ranking would
--     never let govern. Narrowed to protect only an asserted failure.
--   * Both: same-day rows with readings are ordered by entry, not by test
--     time. Documented (R19); the comments no longer claim the as-left
--     test governs.
--
-- ============================================================================
...
BEGIN
    ...
    -- AN ASSERTED FAILURE IS NOT BURIED UNDER FURTHER ASSERTIONS (review
    -- rounds 3–4). The readers' ranking already keeps readings and asserted
    -- failures above every no-readings row, so a parallel row cannot displace
    -- them; round 3's wider refusal also refused legitimate data (two legacy
...
BEGIN
    ...
    -- without, an asserted failure beats any other assertion, and a result
    -- beats no result (round 4); then the stronger record (round 2); only
    -- then the later insertion (round 3). The ranking mirrors what a
    -- correction may and may not replace — a failure is displaced only by
    -- readings. Among same-day rows WITH readings the later ENTRY governs,
    -- which is the later test only if tests are entered in the order they
    -- were performed (residual R19). This ranking, not the insert guard, is
    -- what keeps a parallel row from displacing a failure.
    ...
    v_result := CASE
                    ...
                    ELSE NULL
                END;
    -- Written UNCONDITIONALLY, even when nothing changed (review round 4,
    -- both reviewers): every test insert then leaves a new version of the
    -- meter row, so a concurrent insert running under REPEATABLE READ or
    -- SERIALIZABLE fails with a serialisation error at its meter lock instead
    -- of refreshing the pointer from a snapshot that cannot see this row.
    -- A row-version mutex; no new lock (the row is already held).
...
-- then a row with a result, then the stronger record_basis, then the later
-- insertion (review rounds 2–4) — the same order the pointer uses.
--
...
--
-- R19. SAME-DAY TESTS WITH READINGS ARE RANKED BY ENTRY ORDER. meter_tests
--     has no time of day, so an as-found FAIL and an as-left PASS on one day
--     govern in the order they were entered: entered as-left first, the
--     as-found failure governs. The window date is the same either way; only
--     the result label and prior_test_failed differ. An optional performed_at
--     ranked before recorded_seq is the fix if it matters (review round 4).
--
