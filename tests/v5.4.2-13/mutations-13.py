...
M=[
 ...
 ('M73 second meter at voided premise', "                    OR b.meter_id IN (SELECT d.meter_id FROM public.meter_deployments d\n                                       WHERE d.location_id = r.location_id AND d.tenant_id = NEW.tenant_id", "                    OR b.meter_id IN (SELECT d.meter_id FROM public.meter_deployments d\n                                       WHERE false AND d.tenant_id = NEW.tenant_id", 'P1'),
 ('M74 meter not the customer''s', "                    OR NOT EXISTS (SELECT 1 FROM public.meters m\n                                     JOIN public.service_locations sl ON sl.id = m.location_id", "                    OR false AND NOT EXISTS (SELECT 1 FROM public.meters m\n                                     JOIN public.service_locations sl ON sl.id = m.location_id", 'P1'),
 ('M75 units vs every exceeded bill', "        IF NOT public.backbilling_units_match(v_other, NEW.id) THEN", "        IF false THEN", 'P4'),
]
def run(i, m):
    name, old, new, exp = m
    n = src.count(old)
