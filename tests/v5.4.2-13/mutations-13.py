...
M=[
 ...
 ('M42 withdrawal guard', "            IF OLD.cause = 'meter_error' AND OLD.direction = 'customer_owed'", "            IF false AND OLD.direction = 'customer_owed'", 'I10'),
 ('M43 frozen immutability', "               OR NEW.anchor_date IS DISTINCT FROM OLD.anchor_date\n               OR NEW.claimed_from IS DISTINCT FROM OLD.claimed_from\n               OR NEW.cause_change_reason IS DISTINCT FROM OLD.cause_change_reason", "               OR NEW.cause_change_reason IS DISTINCT FROM OLD.cause_change_reason", 'I2'),
