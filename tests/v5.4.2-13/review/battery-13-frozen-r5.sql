...
-- ======================================================================== N
-- Review round 2 (Fable + Opus, frozen 9844fc48). The two findings that need
-- separate transactions (a removal backdated after the finding; the lock
-- order) are fence-and-race-13.sh D and E.
-- N1: the moment a removal is recorded is the database's.
DO $$ DECLARE r timestamptz; BEGIN
  UPDATE public.meters SET status = 'inactive', removal_date = DATE '2026-05-01' WHERE id = '00000000-0000-4000-8000-000000001392';
  SELECT removal_recorded_at INTO r FROM public.meter_deployments WHERE meter_id = '00000000-0000-4000-8000-000000001392' AND removal_date IS NOT NULL;
  IF r IS DISTINCT FROM now() THEN RAISE EXCEPTION 'N1 FAILED: removal_recorded_at %', r; END IF;
  BEGIN
    UPDATE public.meter_deployments SET removal_recorded_at = '2020-01-01' WHERE meter_id = '00000000-0000-4000-8000-000000001392';
    RAISE EXCEPTION 'N1b FAILED: the removal time was rewritten';
  EXCEPTION WHEN restrict_violation THEN NULL; END;
  BEGIN
    INSERT INTO public.meter_deployments (tenant_id, meter_id, deployment_number, location_id, install_date, removal_recorded_at)
    VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001393', 1, '00000000-0000-4000-8000-0000000013d1', DATE '2026-06-01', '2020-01-01');
    RAISE EXCEPTION 'N1c FAILED: a removal time was supplied at insert';
  EXCEPTION WHEN restrict_violation THEN NULL; END;
  RAISE NOTICE 'PASS N1: a removal written through meters.status (sync_meter_deployments) is stamped with the database clock, and the stamp cannot be supplied or rewritten';
END $$;
-- N2/N3: the reissue gate compares at the scope it matched (O-MAY at L3, $100 on meter ed, voided).
INSERT INTO public.invoices (id, tenant_id, invoice_number, billing_run_id, customer_id, location_id, is_consolidated, invoice_type,
                             invoice_date, billing_period, period_start, period_end, due_date, amount_due) VALUES
  ('00000000-0000-4000-8000-000000001350', '00000000-0000-4000-8000-0000000013a1', 'CONS-MAY', '00000000-0000-4000-8000-000000001306',
   '00000000-0000-4000-8000-0000000013c1', NULL, true, 'regular', '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 180.00),
  ('00000000-0000-4000-8000-000000001351', '00000000-0000-4000-8000-0000000013a1', 'CONS-BARE', '00000000-0000-4000-8000-000000001306',
   '00000000-0000-4000-8000-0000000013c1', NULL, true, 'regular', '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 400.00);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id, usage_quantity) VALUES
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001350', 'gas', 'usage_charge', 'Gas L3', 100.00, '00000000-0000-4000-8000-0000000013ed', 50),
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001350', 'gas', 'usage_charge', 'Gas L1', 80.00, '00000000-0000-4000-8000-0000000013e2', 40);
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001350', '2026-05-31', now());
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001351', '2026-05-31', now());
DO $$ DECLARE st text; BEGIN
  UPDATE public.invoices SET status = 'pending' WHERE id = '00000000-0000-4000-8000-000000001350';
  SELECT status INTO st FROM public.invoices WHERE id = '00000000-0000-4000-8000-000000001350';
  IF st <> 'pending' THEN RAISE EXCEPTION 'N2 FAILED'; END IF;
  RAISE NOTICE 'PASS N2: a consolidated bill carrying the voided meter at the SAME $100 plus another premise''s $80 issues — the gate compares the shared meter, not the whole bill (round 2)';
END $$;
DO $$ BEGIN
  UPDATE public.invoices SET status = 'pending' WHERE id = '00000000-0000-4000-8000-000000001351';
  RAISE EXCEPTION 'N3 FAILED: a lineless consolidated bill re-billed voided days for more';
EXCEPTION WHEN restrict_violation THEN
  RAISE NOTICE 'PASS N3: a consolidated bill with no premise and no lines, charging $400 for days a voided $100 bill covered, is refused — matched by the customer (round 2)'; END $$;
-- N4: unknown usage is not "no usage".
RESET ROLE;
INSERT INTO b13 VALUES ('new_null', pg_temp.bill('00000000-0000-4000-8000-000000001391', '00000000-0000-4000-8000-0000000013c1', '00000000-0000-4000-8000-0000000013d8', '2026-03-01', '2026-03-31', NULL));
SET ROLE tally_app;
DO $$ BEGIN
  INSERT INTO public.meter_correction_evaluations (tenant_id, case_id, submitted_amounts)
  VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-0000000013ac', jsonb_build_object(pg_temp.id('new_null')::text, 0));
  RAISE EXCEPTION 'N4 FAILED: a zero refund on a period of unknown usage';
EXCEPTION WHEN invalid_parameter_value THEN RAISE NOTICE 'PASS N4: a zero correction on a billed period whose usage is unknown (NULL) is refused'; END $$;


-- ======================================================================== O
-- Review round 3 (Fable + Opus, frozen d991df92): both found both holes.
-- O1: at the same premise the whole charge counts — the extra cannot ride on
-- another meter's line or a meterless line (O-MAY at L3, $100 on ed, voided).
SELECT pg_temp.inv('00000000-0000-4000-8000-000000001360', 'O-SPLIT', '00000000-0000-4000-8000-000000001306', 'regular', NULL, '2026-05-01', '2026-05-31', 60.00, 50);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id, usage_quantity)
VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001360', 'gas', 'usage_charge', 'Other meter', 90.00, '00000000-0000-4000-8000-0000000013e2', 10);
UPDATE public.invoices SET amount_due = 150.00 WHERE id = '00000000-0000-4000-8000-000000001360';
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001360', '2026-05-31', now());
SELECT pg_temp.inv('00000000-0000-4000-8000-000000001361', 'O-NOMETER', '00000000-0000-4000-8000-000000001306', 'regular', NULL, '2026-05-01', '2026-05-31', 100.00, 50);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount)
VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001361', 'gas', 'adjustment', 'Usage adjustment', 300.00);
UPDATE public.invoices SET amount_due = 400.00 WHERE id = '00000000-0000-4000-8000-000000001361';
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001361', '2026-05-31', now());
DO $$ DECLARE k text; n int := 0; BEGIN
  FOREACH k IN ARRAY ARRAY['00000000-0000-4000-8000-000000001360', '00000000-0000-4000-8000-000000001361'] LOOP
    BEGIN
      UPDATE public.invoices SET status = 'pending' WHERE id = k::uuid;
      RAISE EXCEPTION 'O1 FAILED: % issued', k;
    EXCEPTION WHEN restrict_violation THEN n := n + 1; END;
  END LOOP;
  RAISE NOTICE 'PASS O1: at the voided bill''s own premise the whole charge counts — $60 on the voided meter plus $90 on another meter, or $100 plus a $300 meterless line, is refused (% of 2)', n;
END $$;
-- O2: a higher voided bill does not raise the ceiling. A $500 duplicate is
-- issued beside a live $100 April bill at L2, both are voided, and a $400
-- rebill of April is refused.
RESET ROLE;
INSERT INTO b13 VALUES ('o2_apr', pg_temp.bill('00000000-0000-4000-8000-0000000013e2', '00000000-0000-4000-8000-0000000013c2', '00000000-0000-4000-8000-0000000013d2', '2026-04-01', '2026-04-30', 50, 100));
INSERT INTO b13 VALUES ('o2_dup', pg_temp.bill('00000000-0000-4000-8000-0000000013e2', '00000000-0000-4000-8000-0000000013c2', '00000000-0000-4000-8000-0000000013d2', '2026-04-01', '2026-04-30', 250, 500));
SET ROLE tally_app;
SELECT public.void_invoice(pg_temp.id('o2_apr'), '00000000-0000-4000-8000-0000000013b1', 'wrong_read', 'battery O2', true);
SELECT public.void_invoice(pg_temp.id('o2_dup'), '00000000-0000-4000-8000-0000000013b1', 'duplicate', 'battery O2', false);
INSERT INTO public.invoices (id, tenant_id, invoice_number, billing_run_id, customer_id, location_id, invoice_type,
                             invoice_date, billing_period, period_start, period_end, due_date, amount_due)
VALUES ('00000000-0000-4000-8000-000000001362', '00000000-0000-4000-8000-0000000013a1', 'O-REBILL', '00000000-0000-4000-8000-000000001306',
        '00000000-0000-4000-8000-0000000013c2', '00000000-0000-4000-8000-0000000013d2', 'regular', '2026-05-01', '2026-04', '2026-04-01', '2026-04-30', '2026-05-21', 400.00);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id, usage_quantity)
VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001362', 'gas', 'usage_charge', 'Gas', 400.00, '00000000-0000-4000-8000-0000000013e2', 90);
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001362', '2026-04-30', now());
DO $$ BEGIN
  UPDATE public.invoices SET status = 'pending' WHERE id = '00000000-0000-4000-8000-000000001362';
  RAISE EXCEPTION 'O2 FAILED: a voided $500 duplicate excused a $400 rebill of $100 days';
EXCEPTION WHEN restrict_violation THEN
  RAISE NOTICE 'PASS O2: a higher voided bill (a $500 duplicate issued beside the live $100 bill, both voided) does not excuse a $400 rebill — more than ANY matched voided bill is an increase'; END $$;
-- O3: a removal written in the same transaction as the test does not tie with
-- it. M-SAME: in service since 2024, billed by Tally from cutover.
RESET ROLE;
INSERT INTO public.meters (id, tenant_id, meter_number, location_id, service_type, start_date, status) VALUES
  ('00000000-0000-4000-8000-000000001394', '00000000-0000-4000-8000-0000000013a1', 'M-SAME', '00000000-0000-4000-8000-0000000013d1', 'gas', DATE '2024-01-01', 'active');
UPDATE public.meter_deployments SET created_at = '2024-01-05' WHERE meter_id = '00000000-0000-4000-8000-000000001394';
SELECT pg_temp.bill('00000000-0000-4000-8000-000000001394', '00000000-0000-4000-8000-0000000013c1', '00000000-0000-4000-8000-0000000013d1', ps, pe)
  FROM (VALUES (DATE '2026-01-15', DATE '2026-01-31'), (DATE '2026-02-01', DATE '2026-02-28'), (DATE '2026-03-01', DATE '2026-03-31'),
               (DATE '2026-04-01', DATE '2026-04-30'), (DATE '2026-05-01', DATE '2026-05-31'), (DATE '2026-06-01', DATE '2026-06-30')) v(ps, pe);
SET ROLE tally_app;
INSERT INTO b13 VALUES ('t_same', pg_temp.rec('00000000-0000-4000-8000-000000001394', DATE '2026-06-15', 103));
INSERT INTO public.meter_correction_cases (id, tenant_id, meter_id, cause, discovering_test_id)
VALUES ('00000000-0000-4000-8000-0000000013ad', '00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001394', 'meter_error', pg_temp.id('t_same'));
SELECT pg_temp.ev('00000000-0000-4000-8000-0000000013ad', ARRAY[-1, -1, -1, -1, -1, -1]);
UPDATE public.meters SET status = 'inactive', removal_date = DATE '2025-12-15' WHERE id = '00000000-0000-4000-8000-000000001394';
DO $$ DECLARE n int; BEGIN
  n := public.meter_correction_uncovered_days((SELECT max(id::text)::uuid FROM public.meter_correction_evaluations WHERE case_id = '00000000-0000-4000-8000-0000000013ad'));
  IF n <> 31 THEN RAISE EXCEPTION 'O3 FAILED: % uncovered days after a same-transaction backdated removal', n; END IF;
  BEGIN
    UPDATE public.meter_correction_cases SET status = 'frozen' WHERE id = '00000000-0000-4000-8000-0000000013ad';
    RAISE EXCEPTION 'O3b FAILED: froze';
  EXCEPTION WHEN restrict_violation THEN NULL; END;
  RAISE NOTICE 'PASS O3: a backdated removal written in the SAME transaction as the fast test does not excuse the 31 unbilled days (application removals must be strictly before the finding)';
END $$;


-- ======================================================================== P
-- Review round 4 (Fable + Opus, frozen ea35e6bb): siblings in the whole-
-- versus-meter split (O-MAY at L3, $100 / 50u on ed, voided).
RESET ROLE;
INSERT INTO public.meters (id, tenant_id, meter_number, location_id, service_type, start_date, status) VALUES
  ('00000000-0000-4000-8000-000000001395', '00000000-0000-4000-8000-0000000013a1', 'M-L3-B', '00000000-0000-4000-8000-0000000013d3', 'gas', DATE '2024-01-01', 'active');
INSERT INTO public.invoices (id, tenant_id, invoice_number, billing_run_id, customer_id, location_id, is_consolidated, invoice_type,
                             invoice_date, billing_period, period_start, period_end, due_date, amount_due) VALUES
  ('00000000-0000-4000-8000-000000001370', '00000000-0000-4000-8000-0000000013a1', 'P-HEADER', '00000000-0000-4000-8000-000000001306',
   '00000000-0000-4000-8000-0000000013c1', '00000000-0000-4000-8000-0000000013d1', false, 'regular', '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 150),
  ('00000000-0000-4000-8000-000000001371', '00000000-0000-4000-8000-0000000013a1', 'P-SECOND', '00000000-0000-4000-8000-000000001306',
   '00000000-0000-4000-8000-0000000013c1', NULL, true, 'regular', '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 400),
  ('00000000-0000-4000-8000-000000001372', '00000000-0000-4000-8000-0000000013a1', 'P-FOREIGN', '00000000-0000-4000-8000-000000001306',
   '00000000-0000-4000-8000-0000000013c1', NULL, true, 'regular', '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 150);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id, usage_quantity) VALUES
  -- P1: the customer's OTHER premise on the header, the extra on another meter
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001370', 'gas', 'usage_charge', 'ed', 60, '00000000-0000-4000-8000-0000000013ed', 50),
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001370', 'gas', 'usage_charge', 'e1', 90, '00000000-0000-4000-8000-0000000013e1', 10),
  -- P2: no premise; ed at the same $100, $300 on a second meter AT L3
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001371', 'gas', 'usage_charge', 'ed', 100, '00000000-0000-4000-8000-0000000013ed', 50),
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001371', 'gas', 'usage_charge', 'L3 meter B', 300, '00000000-0000-4000-8000-000000001395', 150),
  -- P3: no premise; ed at $60, $90 on ANOTHER TENANT's meter
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001372', 'gas', 'usage_charge', 'ed', 60, '00000000-0000-4000-8000-0000000013ed', 50),
  ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001372', 'gas', 'usage_charge', 'foreign', 90, '00000000-0000-4000-8000-0000000013e0', 10);
SELECT pg_temp.snap(id, '2026-05-31', now()) FROM public.invoices
 WHERE id IN ('00000000-0000-4000-8000-000000001370', '00000000-0000-4000-8000-000000001371', '00000000-0000-4000-8000-000000001372');
SET ROLE tally_app;
DO $$ DECLARE k text; n int := 0; BEGIN
  FOREACH k IN ARRAY ARRAY['00000000-0000-4000-8000-000000001370', '00000000-0000-4000-8000-000000001371', '00000000-0000-4000-8000-000000001372'] LOOP
    BEGIN
      UPDATE public.invoices SET status = 'pending' WHERE id = k::uuid;
      RAISE EXCEPTION 'P1-3 FAILED: % issued', k;
    EXCEPTION WHEN restrict_violation THEN n := n + 1; END;
  END LOOP;
  RAISE NOTICE 'PASS P1: a bill naming the customer''s other premise compares whole ($150 against $100 refused); a bill with no premise counts a second meter AT the voided premise ($400) and a meter not the customer''s ($150) — all refused (% of 3)', n;
END $$;
-- P4: the R-39 units test reads every voided bill the correction exceeds.
RESET ROLE;
INSERT INTO public.invoices (id, tenant_id, invoice_number, billing_run_id, customer_id, location_id, invoice_type,
                             invoice_date, billing_period, period_start, period_end, due_date, amount_due)
VALUES ('00000000-0000-4000-8000-000000001373', '00000000-0000-4000-8000-0000000013a1', 'P-V', '00000000-0000-4000-8000-000000001306',
        '00000000-0000-4000-8000-0000000013c1', '00000000-0000-4000-8000-0000000013d1', 'regular', '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 100);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id, usage_quantity)
VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001373', 'gas', 'usage_charge', 'Gas', 100, '00000000-0000-4000-8000-0000000013ed', 90);
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001373', '2026-05-31', now());
INSERT INTO public.billing_runs (id, tenant_id, run_number, billing_period, period_start, period_end, run_type, status, correction_rate_mode, started_at) VALUES
  ('00000000-0000-4000-8000-000000001307', '00000000-0000-4000-8000-0000000013a1', 'RC-P4', '2026-06', '2026-06-01', '2026-06-30', 'correction', 'in_progress', 'historical', now());
SET ROLE tally_app;
UPDATE public.invoices SET status = 'pending' WHERE id = '00000000-0000-4000-8000-000000001373';
SELECT public.void_invoice('00000000-0000-4000-8000-000000001373', '00000000-0000-4000-8000-0000000013b1', 'wrong_rate', 'battery P4', true);
INSERT INTO public.correction_run_targets (tenant_id, billing_run_id, voided_invoice_id, meter_id, customer_id, location_id, rate_date_mode, backbill_cause)
VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001307', '00000000-0000-4000-8000-000000001373',
        '00000000-0000-4000-8000-0000000013ed', '00000000-0000-4000-8000-0000000013c1', '00000000-0000-4000-8000-0000000013d1', 'historical', 'rate_misapplication');
RESET ROLE;
INSERT INTO public.invoices (id, tenant_id, invoice_number, billing_run_id, customer_id, location_id, invoice_type, replaces_invoice_id,
                             invoice_date, billing_period, period_start, period_end, due_date, amount_due)
VALUES ('00000000-0000-4000-8000-000000001374', '00000000-0000-4000-8000-0000000013a1', 'P-C', '00000000-0000-4000-8000-000000001307',
        '00000000-0000-4000-8000-0000000013c1', '00000000-0000-4000-8000-0000000013d3', 'correction', '00000000-0000-4000-8000-000000001373',
        '2026-06-01', '2026-05', '2026-05-01', '2026-05-31', '2026-06-21', 400);
INSERT INTO public.invoice_line_items (tenant_id, invoice_id, service_type, charge_type, description, amount, meter_id, usage_quantity)
VALUES ('00000000-0000-4000-8000-0000000013a1', '00000000-0000-4000-8000-000000001374', 'gas', 'usage_charge', 'Gas', 400, '00000000-0000-4000-8000-0000000013ed', 90);
SELECT pg_temp.snap('00000000-0000-4000-8000-000000001374', '2026-05-31', now());
SET ROLE tally_app;
DO $$ BEGIN
  UPDATE public.invoices SET status = 'pending' WHERE id = '00000000-0000-4000-8000-000000001374';
  RAISE EXCEPTION 'P4 FAILED: a correction of an L1 bill, addressed to L3, billed 90 units over voided O-MAY''s 50';
EXCEPTION WHEN restrict_violation THEN
  IF SQLERRM NOT LIKE '%which it does not replace%' THEN RAISE EXCEPTION 'P4 FAILED for the wrong reason: %', SQLERRM; END IF;
  RAISE NOTICE 'PASS P4: a rate_misapplication correction replacing one bill but exceeding another voided bill must also match THAT bill''s units (R-39)'; END $$;
