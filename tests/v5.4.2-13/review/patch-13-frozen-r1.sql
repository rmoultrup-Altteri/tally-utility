...
--   bound from deployments recorded before the finding; the billing periods
--   from the invoices. The caller supplies the cause (a finding, gated where
--   it lifts protection), the dollar figure per period (sign-checked), and
--   nothing else that moves the window.
--
...
--   14. STANDING SURFACES: forfeitures, open holds, case status.
...
--
-- Three guards, for tally_app (the owner — migrations, onboarding — is
-- exempt):
...
BEGIN
    ...
    IF NEW.meter_id     IS DISTINCT FROM OLD.meter_id
       ...
       OR NEW.tenant_id    IS DISTINCT FROM OLD.tenant_id THEN
        RAISE EXCEPTION USING
            MESSAGE = format('meter deployment %s: the meter, the location, the install date and the recorded time are fixed once entered — a deployment''s start bounds a §7.45 correction window (R-37(a)), so moving it would move a refund (v5.4.2-13)', OLD.id),
...
COMMENT ON FUNCTION public.enforce_meter_deployment_history() IS
    'v5.4.2-13 (R-37(a)). BEFORE INSERT OR UPDATE on meter_deployments, for tally_app: created_at stamped from the database clock on insert; meter_id, location_id, install_date, created_at and tenant_id immutable; removal_date and removal_reason written once. A deployment''s start bounds a correction window, so these are evidence.';
...
-- that charges more for a premise and period already billed on a bill since
-- voided is a backbilling correction whatever its invoice_type. It may
...
BEGIN
    ...
    -- Was this premise and period billed before, on a bill since voided?
    -- The HIGHEST prior charge, so a chain of void-and-reissue cannot walk
    ...
    IF v_prior_charge IS NULL OR v_new_charge <= v_prior_charge THEN
    ...
    IF NEW.invoice_type <> 'correction' OR NEW.replaces_invoice_id IS NULL THEN
        RAISE EXCEPTION USING
            MESSAGE = format('backbilling: invoice %s charges %s for %s..%s at this premise, which was already billed %s on a bill since voided — charging more for a period already billed is a backbilling correction whatever the invoice is typed (invoice_type %s)', NEW.invoice_number, v_new_charge, NEW.period_start, NEW.period_end, v_prior_charge, NEW.invoice_type),
            ERRCODE = 'restrict_violation',
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.status = 'frozen' THEN
            ...
            -- recompute (R-38). Nothing else may change with it.
            IF NEW.status <> 'open'
               OR NEW.cause IS DISTINCT FROM OLD.cause
               OR NEW.discovering_test_id IS DISTINCT FROM OLD.discovering_test_id
               OR NEW.anchor_date IS DISTINCT FROM OLD.anchor_date
               OR NEW.claimed_from IS DISTINCT FROM OLD.claimed_from
               OR NEW.cause_change_reason IS DISTINCT FROM OLD.cause_change_reason
               OR NEW.withdrawn_reason IS DISTINCT FROM OLD.withdrawn_reason
               OR NEW.tamper_evidence_kind IS DISTINCT FROM OLD.tamper_evidence_kind
        ...
        IF NEW.status = 'frozen' THEN
            IF NEW.cause IS DISTINCT FROM OLD.cause
               OR NEW.discovering_test_id IS DISTINCT FROM OLD.discovering_test_id
               OR NEW.anchor_date IS DISTINCT FROM OLD.anchor_date
               OR NEW.claimed_from IS DISTINCT FROM OLD.claimed_from THEN
                RAISE EXCEPTION USING
                    MESSAGE = format('meter correction case %s: freezing and changing what was evaluated in one statement would freeze evidence for a case that no longer exists — change, re-evaluate, then freeze', OLD.id),
                    ERRCODE = 'restrict_violation';
            END IF;
        ...
        IF NEW.status = 'withdrawn' THEN
            IF OLD.cause = 'meter_error' AND OLD.direction = 'customer_owed'
               AND NOT EXISTS (SELECT 1 FROM public.meter_tests s
                                WHERE s.supersedes_test_id = OLD.discovering_test_id) THEN
                RAISE EXCEPTION USING
                    MESSAGE = format('meter correction case %s: a fast-meter correction cannot be withdrawn while the test that found it stands — the refund back to the window is a duty §7.45(7)(B)(v)(I) does not let the utility decline (R-37), and a withdrawal would be the decline', OLD.id),
                    ERRCODE = 'restrict_violation',
                    HINT = 'If the test was wrong, correct it in meter_tests (a superseding row); the case may then be withdrawn. A period with no Tally bill is held (meter_correction_holds), not withdrawn.';
            END IF;
            RETURN NEW;
        END IF;
...
--     deployments RECORDED no later than the discovering test was — a
--     deployment entered after the finding cannot shorten its window
--     (section 4). None recorded → no bound. A deployment start BOUNDS the
...
--     that premise and came out on or before this one went in. Every meter's
...
BEGIN
    ...
    END IF;
    SELECT t.recorded_at INTO v_test FROM public.meter_tests t
     WHERE t.id = c.discovering_test_id AND t.tenant_id = c.tenant_id;
    SELECT d.id, d.install_date, d.location_id INTO v_dep
     ...
     WHERE v_stat IS NOT NULL
       ...
       AND d.created_at <= v_test.recorded_at
    ...
    v_dep_corroborated := v_dep.id IS NOT NULL AND EXISTS (
        SELECT 1 FROM public.meter_deployments pd
         WHERE pd.tenant_id = c.tenant_id
           ...
           AND pd.created_at <= v_test.recorded_at
    ...
    RETURN jsonb_build_object(
        ...
        'discovering_test_id', c.discovering_test_id, 'anchor_date', c.anchor_date,
...
CREATE TABLE IF NOT EXISTS public.meter_correction_holds (
    ...
    CONSTRAINT meter_correction_holds_no_overlap
...
BEGIN
    ...
    IF NEW.hold_code = 'legacy_records_not_loaded' THEN
        ...
        END IF;
    ELSE
        -- The meter's deployment covering the whole stretch, recorded, at a
        -- premise acquired from a prior owner after the stretch ended.
...
BEGIN
    ...
    IF e.id IS NULL OR e.cause <> 'meter_error' OR e.direction <> 'customer_owed' OR e.window_start IS NULL THEN
        RETURN 0;
    END IF;
    SELECT t.recorded_at INTO v_rec FROM public.meter_tests t WHERE t.id = e.discovering_test_id;
...
CREATE TRIGGER b_enforce_test_supersession_vs_frozen_case BEFORE INSERT ON public.meter_tests
    FOR EACH ROW EXECUTE FUNCTION public.enforce_test_supersession_vs_frozen_case();
ALTER TABLE public.meter_tests ENABLE ALWAYS TRIGGER b_enforce_test_supersession_vs_frozen_case;
...
--     CORROBORATED — an engineering reading of Kyle's ruling, for him to
--     confirm. Found while building the battery: sync_meter_deployments()
--     creates every meter's first deployment from meters.start_date, which
--     defaults to CURRENT_DATE, so on a migrated meter "the start of its
--     deployments" is its onboarding date. Honoured as written, R-37(a)
--     would end every fast-meter refund at go-live. The bound therefore
--     shortens a refund only where another meter served that premise and was
--     removed on or before this meter's install; it shortens a charge
--     whenever a deployment is recorded. Cost: a genuinely new meter at a
--     brand-new premise with a migrated start date reaches back further for
--     a refund than R-37(a) requires — toward the customer, and bounded by
--     six months. Deployments entered after the discovering test never bound
--     its window; existing rows keep whatever created_at they had.
--
...
--     hold and freeze all serialise on the case row (FOR NO KEY UPDATE / the
--     UPDATE itself), so none races another. A bill issued, a deployment
--     entered or a test recorded on the meter at the same moment as a
--     freeze is not locked out: the case freezes on the evidence it could
--     see, and meter_correction_case_status then reads evaluation_current =
--     false. -14's posting must re-derive the fingerprint and refuse a stale
--     frozen case — the freeze is a checkpoint, not the last check.
--
-- R17. A FAST METER FOUND TAMPERED LOSES ITS REFUND REACH. meter_error may
--     become tampering_bypass (R-19's progression, R-38's gate); tampering is
--     billable-uncapped with a per-period direction, so a fast meter's
--     mandatory refund and under-reach check no longer apply. The supervisor
--     and the evidence reference are the control Kyle ruled; a tamper that
--     makes a meter run FAST against its own customer is rare enough to
--     record rather than rule on.
--
-- R16. THE CORROBORATING PREDECESSOR'S REMOVAL IS WRITE-ONCE BUT NOT
--     TIME-STAMPED. A deployment counts as corroboration only if it was
--     recorded before the finding, but its removal_date may be written
--     later (NULL → value, once). Writing a false removal onto another
--     meter's deployment to shorten a refund would falsify that meter's own
--     history in the same act; recorded, not closed.
--
-- ----------------------------------------------------------------------------
-- 16. The AC-32 tail
-- ----------------------------------------------------------------------------
-- Seven new tenant tables and three views, each born leaky by the default
