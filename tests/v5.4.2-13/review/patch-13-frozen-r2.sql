...
--     or a predecessor meter (review round 1, Fable).
...
BEGIN
    ...
    SELECT max(public.backbilling_invoice_charge(i.id)) INTO v_prior_charge
     ...
     WHERE i.tenant_id = NEW.tenant_id
       ...
       AND ((NEW.replaces_invoice_id IS NOT NULL AND i.id = NEW.replaces_invoice_id)
         -- Any OVERLAPPING period, found by the premise OR by a meter the two
         -- bills share (review round 1, both reviewers). Matching the exact
         -- period at the same location_id let a bill shifted by a day, one
         -- spanning two voided months, one addressed to the customer's other
         -- premise, or a consolidated bill with no location re-bill voided
         -- days for more — the header is the caller's, the meter is the act.
    ...
    IF NEW.invoice_type <> 'correction' OR NEW.replaces_invoice_id IS NULL THEN
        RAISE EXCEPTION USING
            MESSAGE = format('backbilling: invoice %s charges %s for %s..%s, days already billed %s (same premise or same meter) on a bill since voided — charging more for days already billed is a backbilling correction whatever the invoice is typed or addressed (invoice_type %s)', NEW.invoice_number, v_new_charge, NEW.period_start, NEW.period_end, v_prior_charge, NEW.invoice_type),
            ERRCODE = 'restrict_violation',
...
BEGIN
    ...
    FOR v_key, v_val IN SELECT k, v FROM jsonb_each(NEW.submitted_amounts) AS e(k, v) LOOP
        ...
        IF c.direction IS NOT NULL AND v_amount = 0
           AND coalesce((SELECT (x ->> 'billed_units')::numeric
                           FROM jsonb_array_elements(v_inputs -> 'periods') AS x
                          WHERE x ->> 'invoice_id' = v_key), 0) <> 0 THEN
            RAISE EXCEPTION USING
...
BEGIN
    ...
    -- committed. The meter row this insert already holds is not one a freeze
    -- takes, so the lock order cannot invert.
...
--     test's chain, or any defective test within the window's reach) — so
--     re-pointing a case at a fresh copy of its test no longer admits
--     evidence written after the finding (round 1, Opus). And the
--     application cannot insert a deployment already removed (section 4). A
--     history fabricated BEFORE any defective test is recorded — an operator
--     who installs a phantom deployment, closes it, and only then records the
--     test — is not caught by any date; it needs two writes against the
--     meter's own history that the owner can audit, and is recorded here.
--     Existing deployment rows keep whatever created_at they had.
...
--     rests on waits on the meter's cases FOR SHARE and then sees the freeze
--     (round 1). What is still not locked out: a bill issued, a deployment
--     entered or an unrelated test recorded on the meter at the same moment
--     as a freeze. The case freezes on the evidence it could see and
--     meter_correction_case_status then reads evaluation_current = false.
--     -14's posting must re-derive the fingerprint and refuse a stale frozen
--     case — the freeze is a checkpoint, not the last check.

-- R16. A LIVE DEPLOYMENT'S REMOVAL IS WRITE-ONCE BUT NOT TIME-FENCED.
--     The application cannot insert a deployment already removed, but it
--     removes live ones (NULL → date, once), and that removal_date is not
--     itself stamped. Writing a false removal onto another meter's live
--     deployment to corroborate a shorter refund would falsify that meter's
--     own history in the same act; recorded, not closed.
--
...
--
-- R20. meter_deployments' OWN LINKS ARE TENANT-BLIND. Its meter_id and
...
--
-- R21. THE REISSUE GATE REFUSES A SPANNING BILL AFTER A VOID. It matches any
--     overlapping period on the same premise or a shared meter (round 1), so
--     an ordinary bill covering a voided month and a new one, charging more
--     than the voided bill alone, is refused. The voided period is billed as
--     its own correction.
--
-- ----------------------------------------------------------------------------
-- 16. The AC-32 tail
