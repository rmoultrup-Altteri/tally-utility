...
BEGIN
    ...
    -- Money on the new bill that no line carries (amount_due above the
    -- lines). It cannot be attributed to a meter, so it is counted against
    -- every scope: a caller cannot hide an increase by leaving it off the
    -- lines.
    v_new_excess := greatest(0::numeric,
    ...
    -- both reviewers): a consolidated bill that carries the voided meter at
    -- the same price and another premise besides is not charging more for
    -- the voided days. Shared meters → the two bills' lines on those meters
    -- (plus the new bill's unattributed money); otherwise the whole charge,
    -- the GREATER of lines and amount_due on each side. Deliberately NOT per
    -- day: prorating lets a cheap new month dilute an overcharge below the
    -- old daily rate. A bill is a rebill that charges more when it charges
    -- more than EVERY earlier bill it matches — the highest, so a chain of
    -- void-and-reissue cannot walk the figure up one bill at a time.
    FOR r IN
        SELECT i.id,
    ...
    LOOP
        v_matched := true;
        IF r.id IS DISTINCT FROM NEW.replaces_invoice_id AND r.shares_meter THEN
        ...
        IF v_new_part <= v_old_part THEN
            v_increase := false;               -- holds or reduces against this bill
        ELSIF v_increase IS NULL OR v_increase THEN
            v_increase := true;
    ...
    v_dep_corroborated := v_dep.id IS NOT NULL AND EXISTS (
        SELECT 1 FROM public.meter_deployments pd
         WHERE pd.tenant_id = c.tenant_id
           ...
           AND coalesce(pd.removal_recorded_at, pd.created_at) <= v_fence
...
BEGIN
    ...
    SELECT count(*) INTO v_n
     ...
     WHERE NOT EXISTS (SELECT 1 FROM public.meter_correction_period_evidence p
       ...
       -- turned the unbilled stretch into a "gap").
       AND NOT (
...
--     round 1's order deadlocked). What is still not locked out: a bill
