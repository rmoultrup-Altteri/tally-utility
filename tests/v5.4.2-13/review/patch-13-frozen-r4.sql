...
BEGIN
    ...
    -- — the two bills' lines on their shared meters, plus the new bill's
    -- money no meter carries — only for a match through a shared meter at
    -- another premise, or with no premise. Deliberately NOT per day:
    ...
    LOOP
        ...
        IF r.id IS DISTINCT FROM NEW.replaces_invoice_id AND r.shares_meter
           AND (NEW.location_id IS NULL OR r.location_id IS DISTINCT FROM NEW.location_id) THEN
